...
#
#	Program:	poDiff
#	Copyright:	Paul Obermeier 1999-2005
#	Filename:	poDiff.tcl
#
...
InitPackages Tk tablelist poTcllib poTklib poExtlib
...
proc GetRootDir { w } {
    ...
    if { ($w == $gPo(leftList)) || \
         ($w == $gPo(diffListL)) || \
         ([info exists gPo(ignListL)]   && $w == $gPo(ignListL)) || \
         ([info exists gPo(identListL)] && $w == $gPo(identListL)) } {
	return $gPo(dir1)
    } else {
	return $gPo(dir2)
    }
...
proc AbsPath { fileName rootName } {
	...
	# Join root pathname and relative filename (remove "./" from filename)
	set name [file join $rootName [string range $name 2 end]]
    }
    return $name
}

proc StartHexEditor { w serialize } {
    set fileList [list]
    ...
    ::poExtProg::StartHexEditProg $fileList ::WriteInfoStr $serialize
}
...
proc StartAssoc { w serialize } {
    StartEditor $w $serialize 1
}

proc StartEditor { w serialize { assoc 0 } } {
    set fileList [list]
    set selItemList [GetListSelection $w]
    if { [llength $selItemList] == 0 } {
        return
    ...
    ::poExtProg::StartEditProg $fileList ::WriteInfoStr $serialize $assoc
}

proc StartGUIDiff { leftListbox rightListbox { serialize 0 } } {
    global gPo

    if { $leftListbox eq "" || \
         [winfo class $leftListbox] ne "Listbox" } {
    	return
    }

    set fileList1 {}
    set fileList2 {}
    set indList1 [$leftListbox  curselection]
    set indList2 [$rightListbox curselection]
    if { [llength $indList1] == 0 || [llength $indList2] == 0 } {
        return
    }
    foreach ind1 $indList1 ind2 $indList2 {
	# Convert native name back to Unix notation
	set fileName1 [AbsPath [$leftListbox  get $ind1] $gPo(dir1)]
	set fileName2 [AbsPath [$rightListbox get $ind2] $gPo(dir2)]
	lappend fileList1 $fileName1
	lappend fileList2 $fileName2
    }
    ::poExtProg::StartDiffProg $fileList1 $fileList2 ::WriteInfoStr $serialize
}
...
proc DiffFiles {} {
    ...
    set prog ""
    if { $gPo(leftFile) eq "" || $gPo(rightFile) eq "" } {
	set msg "No files marked for diff'ing.\nSee help about marking files."
	tk_messageBox -title "Info" -message $msg -type ok -icon warning
	return	
    }

    ::poExtProg::StartDiffProg [list $gPo(leftFile)] [list $gPo(rightFile)] \
                               ::WriteInfoStr 0
}

proc FindListId { X Y } {
    set id [winfo containing $X $Y]
    if { [winfo class $id] eq "Listbox" } {
        return $id
    } else {
        while { [winfo class $id] ne "Tablelist" } {
            set id [file rootname $id]
        }
        return $id
    }
}
...
proc SelectAll { X Y } {
    ...
    set listboxId [FindListId $X $Y]
    $listboxId selection set 0 end
    if { $listboxId eq $gPo(diffListL) } {
        SynchSelections $listboxId $gPo(diffListR)
     } elseif { $listboxId eq $gPo(diffListR) } {
        SynchSelections $listboxId $gPo(diffListL)
     } elseif { [info exists gPo(identListL)] && \
                $listboxId eq $gPo(identListL) } {
        SynchSelections $listboxId $gPo(identListR)
     } elseif { [info exists gPo(identListR)] && \
                $listboxId eq $gPo(identListR) } {
        SynchSelections $listboxId $gPo(identListL)
    }
}
...
proc SynchSelections { fromListbox toListbox } {
    ...
    set indList [$fromListbox curselection]
    $toListbox selection clear 0 end
    foreach ind $indList {
	$toListbox selection set $ind
    }
    if { $fromListbox eq $gPo(diffListL) || \
         $fromListbox eq $gPo(diffListR) } {
        DisplayTwoFileInfo $gPo(diffListL)  $gPo(diffListR) \
                           $gPo(infoFrameL) $gPo(infoFrameR)
    }
}

proc TimeReplaceDir { searchPatt { replacePatt "" } } {
    global gPo

    if { $replacePatt ne "" } {
	set retVal [tk_messageBox \
	  -title "Replace confirmation" \
	  -message "This operation cannot be undone. Replace ?" \
	  -type yesno -default no -icon question]
	if { $retVal eq "no" } {
	    return
	}
        set replacePattStr $gPo($replacePatt)
    } else {
        set replacePattStr ""
    }

    set gPo(curSearchPatt) $gPo($searchPatt)
    set timeStr [time "ReplaceDir [list $gPo($searchPatt)] [list $replacePattStr]"]
    scan $timeStr "%d" ms
    set sec [format "%4.1f" [expr $ms / 1000.0 / 1000.0]]
    AppendInfoStr " (Elapsed time: $sec seconds)"
}
...
proc DeleteMenuEntries { menuId startInd } {
    set endInd [$menuId index end]
    if { $endInd >= $startInd } {
	$menuId delete $startInd $endInd
    }
}

proc ClearRecentDirList { leftMenuId rightMenuId } {
    global gPo 

    set gPo(dirList) {}
    DeleteMenuEntries $leftMenuId  2
    DeleteMenuEntries $rightMenuId 2
}

proc AddRecentDirs { menuId side delOldEntries } {
    global gPo

    if { $delOldEntries } {
	DeleteMenuEntries $menuId 2
    }
    for { set i 0 } { $i < [llength $gPo(dirList)] } { incr i } {
        set curDir [lindex $gPo(dirList) $i]
	AddMenuCmd $menuId $curDir "" "SetDir [list $curDir] $side"
    }
}

proc SelectSession { sessionName } {
    global gPo

    foreach session [lsort [array names gPo "session,$sessionName,*"]] {
	set varName [lindex [split $session ","] 2]
	set gPo($varName) $gPo(session,$sessionName,$varName)
    }
    UpdateMainTitle
    # Fill the text widgets of compare settings window from the ignore lists.
    if { [info exists gPo(ignDirText)] && [winfo exists $gPo(ignDirText)] } {
	$gPo(ignDirText) delete 0.1 end
	foreach d $gPo(ignDirList) {
	    $gPo(ignDirText) insert end "$d\n"
	}
    }
    if { [info exists gPo(ignFileText)] && [winfo exists $gPo(ignFileText)] } {
	$gPo(ignFileText) delete 0.1 end
	foreach f $gPo(ignFileList) {
	    $gPo(ignFileText) insert end "$f\n"
	}
    }
    set gPo(curSession) $sessionName
}

proc ClearRecentSessionList { menuId } {
    global gPo 

    foreach session [array names gPo "session,*,*"] {
	unset gPo($session)
    }
    DeleteMenuEntries $menuId 0
}

proc AddRecentSessions { menuId delOldEntries } {
    global gPo

    if { $delOldEntries } {
	DeleteMenuEntries $menuId 0
    }
    foreach session [lsort [array names gPo "session,*,dir1"]] {
	set sessionName [lindex [split $session ","] 1]
	AddMenuCmd $menuId $sessionName "" "SelectSession [list $sessionName]"
    }
}
...
proc AskSaveSession {} {
    ...
    ::poWin::ShowEntryBox SaveSession $gPo(curSession) \
		          "New Session" "Enter new session name"
}

proc SaveSession { val } {
    global gPo

    foreach session [lsort [array names gPo "session,*,dir1"]] {
	set sessionName [lindex [split $session ","] 1]
	if { [string equal $sessionName $val] } {
	    set retVal [tk_messageBox -icon question -type yesno -default yes \
    ...
    set gPo(curSession) $val

    # Directories to compare.
    set gPo(session,$val,dir1) $gPo(dir1)
    set gPo(session,$val,dir2) $gPo(dir2)
    # Settings from compare window.
    set gPo(session,$val,cmpMode)        $gPo(cmpMode)
    set gPo(session,$val,ignEolChar)     $gPo(ignEolChar)
    set gPo(session,$val,ignHiddenDirs)  $gPo(ignHiddenDirs)
    set gPo(session,$val,ignHiddenFiles) $gPo(ignHiddenFiles)
    set gPo(session,$val,ignDirList)     $gPo(ignDirList)
    set gPo(session,$val,ignFileList)    $gPo(ignFileList)

    # Add this session name to the session menu.
    AddRecentSessions $gPo(sessionMenu) true
}

proc InitFileInfoLabels {} {
    global gPo

    set row 0
    foreach fileLabel [::poMisc::GetFileInfoLabels] {
        $gPo(infoFrameL).key_$row configure -text $fileLabel
        $gPo(infoFrameL).val_$row configure -text ""
        $gPo(infoFrameR).key_$row configure -text $fileLabel
        $gPo(infoFrameR).val_$row configure -text ""
        incr row
    }
}
...
proc ShowMainWin {title} {
    ...
    wm title . $title
    wm geometry . [format "%dx%d+%d+%d" $gPo(main,w) $gPo(main,h) \
    ...
    set gPo(paneWin) .fr.workfr.fr.pane
    panedwindow $gPo(paneWin) -orient vertical -sashpad 2 -sashwidth 2 \
                -opaqueresize true
    pack $gPo(paneWin) -side top -expand 1 -fill both

    set tf .fr.workfr.fr.pane.tfr
    set bf .fr.workfr.fr.pane.bfr
    set if .fr.workfr.fr.pane.ifr
    frame $tf
    frame $bf
    frame $if
    ...
    $gPo(paneWin) add $tf $bf $if
    update
    $gPo(paneWin) sash place 0 0 $gPo(sashY0)
    $gPo(paneWin) sash place 1 0 $gPo(sashY1)

    frame $tf.leftfr  -relief raised -borderwidth 1
    frame $tf.rightfr -relief raised -borderwidth 1
    frame $bf.difffr  -relief raised -borderwidth 1
    frame $if.leftfr  -relief raised -borderwidth 1
    frame $if.rightfr -relief raised -borderwidth 1
    ...
    set gPo(leftMenu)    $leftMenu
    set gPo(rightMenu)   $rightMenu
    set gPo(sessionMenu) $sessionMenu

    menu $fileMenu -tearoff 0
    $fileMenu add cascade -label "Select left dir"  -menu $fileMenu.left
    $fileMenu add cascade -label "Select right dir" -menu $fileMenu.right
    $fileMenu add cascade -label "Select session"   -menu $fileMenu.session

    menu $leftMenu -tearoff 0
    AddMenuCmd $leftMenu "Browse ..."  "Ctrl+L" "ChooseDirs 1"
    $leftMenu add separator
    bind . <Control-l>  "ChooseDirs 1"
    AddRecentDirs $leftMenu 1 false

    menu $rightMenu -tearoff 0
    AddMenuCmd $rightMenu "Browse ..." "Ctrl+R" "ChooseDirs 2"
    $rightMenu add separator
    bind . <Control-r>  "ChooseDirs 2"  
    AddRecentDirs $rightMenu 2 false

    menu $sessionMenu -tearoff 0
    AddRecentSessions $sessionMenu false

    AddMenuCmd $fileMenu "Save as session ..." "Ctrl+S" AskSaveSession
    $fileMenu add separator
    AddMenuCmd $fileMenu "Diff directories (Update)" "F5"     TimeDiffDir
    AddMenuCmd $fileMenu "Diff selected files"       "Ctrl+D" DiffFiles
    $fileMenu add separator
    AddMenuCmd $fileMenu "Close info windows"  "Ctrl+W" CloseFileInfoWins
    AddMenuCmd $fileMenu "Quit"                "Ctrl+Q" ExitProg
    bind . <Control-s>  AskSaveSession
    bind . <Key-F5>     TimeDiffDir
    bind . <Control-d>  DiffFiles
    bind . <Control-w>  CloseFileInfoWins
    bind . <Control-q>  ExitProg
    wm protocol . WM_DELETE_WINDOW "ExitProg"
    bind . <Escape>  StopScan

    set editMenu $hMenu.edit
    menu $editMenu -tearoff 0
    AddMenuCmd $editMenu "Search/Replace" "F3"  ShowReplaceWin
    bind . <Key-F3> ShowReplaceWin

    set viewMenu $hMenu.view
    menu $viewMenu -tearoff 0
    AddMenuCmd $viewMenu "Error log"       "F6"     ShowErrorLog
    AddMenuCmd $viewMenu "Ignored files"   "F7"     ShowIgnoreLog
    AddMenuCmd $viewMenu "Identical files" "F8"     ShowIdentLog
    bind . <Key-F6>    ShowErrorLog
    bind . <Key-F7>    ShowIgnoreLog
    bind . <Key-F8>    ShowIdentLog

    set settMenu $hMenu.sett
    menu $settMenu -tearoff 0
    AddMenuCmd $settMenu "Logging ..."     "" ::poLogOpt::OpenWin
    AddMenuCmd $settMenu "Misc ..."        "" ShowMiscSettWin
    AddMenuCmd $settMenu "File types ..."  "" ShowFileTypeSettWin
    AddMenuCmd $settMenu "Image types ..." "" ShowImgTypeSettWin
    AddMenuCmd $settMenu "Compare ..."     "" ShowCompareSettWin
    ...
    set gPo(infoFrameL) [::poWin::CreateScrolledFrame $if.leftfr  "Left info"]
    set gPo(infoFrameR) [::poWin::CreateScrolledFrame $if.rightfr "Right info"]
    set colors [list "gray" "white"]
    set row 0
    foreach fileLabel [::poMisc::GetFileInfoLabels] {
        set cind [expr $row % [llength $colors]]
        label $gPo(infoFrameL).key_$row -anchor w -bg [lindex $colors $cind]
        label $gPo(infoFrameL).val_$row -anchor w -bg [lindex $colors $cind]
        grid $gPo(infoFrameL).key_$row -column 0 -row $row -sticky new
        grid $gPo(infoFrameL).val_$row -column 1 -row $row -sticky new

        label $gPo(infoFrameR).key_$row -anchor w -bg [lindex $colors $cind]
        label $gPo(infoFrameR).val_$row -anchor w -bg [lindex $colors $cind]
        grid $gPo(infoFrameR).key_$row -column 0 -row $row -sticky new
        grid $gPo(infoFrameR).val_$row -column 1 -row $row -sticky new
        incr row
    }
    InitFileInfoLabels
    grid columnconfigure $gPo(infoFrameL) 0 -weight 1
    grid columnconfigure $gPo(infoFrameL) 1 -weight 1
    grid columnconfigure $gPo(infoFrameR) 0 -weight 1
    grid columnconfigure $gPo(infoFrameR) 1 -weight 1

    set frameList [list \
        [list leftList  $tf.leftfr  "Only in left directory"  $gPo(infoFrameL)] \
	[list rightList $tf.rightfr "Only in right directory" $gPo(infoFrameR)]]
    foreach frameEntry $frameList { 
    	set gPo([lindex $frameEntry 0]) \
	    [::poWin::CreateScrolledListbox \
		[lindex $frameEntry 1] \
		[lindex $frameEntry 2] \
		-selectmode extended -exportselection false -bg white]
	
        set listboxId $gPo([lindex $frameEntry 0])
        set infoFrame [lindex $frameEntry 3]
        bind $listboxId <<ListboxSelect>> \
                "DisplayFileInfo $listboxId $infoFrame"
	bind $listboxId <1> "+focus $listboxId"
	bind $listboxId <Double-1> "StartAssoc $listboxId 0"
	bind $listboxId <Button-3> \
	    "OpenMainContextMenu $listboxId \"\" %X %Y"
	if { $tcl_platform(platform) eq "windows" } {
	    bind $listboxId <App> \
		"OpenMainContextMenu $listboxId \"\" %X %Y"
	}
    ...
    set listboxList \
        [::poWin::CreateSyncListbox \
   	    $bf.difffr "Different left" "Different right" \
	    -selectmode extended -exportselection false -bg white]
    set gPo(diffListL) [lindex $listboxList 0]
    set gPo(diffListR) [lindex $listboxList 1]
	
    bind $gPo(diffListL) <1> "+focus $gPo(diffListL)"
    bind $gPo(diffListL) <Any-KeyRelease> \
	"SynchSelections $gPo(diffListL) $gPo(diffListR)"
    bind $gPo(diffListL) <ButtonRelease-1> \
	"SynchSelections $gPo(diffListL) $gPo(diffListR)"
    bind $gPo(diffListL) <Double-1> \
	"StartGUIDiff $gPo(diffListL) $gPo(diffListR)"
    bind $gPo(diffListL) <Button-3> \
	"OpenMainContextMenu $gPo(diffListL) $gPo(diffListR) %X %Y"

    bind $gPo(diffListR) <1> "+focus $gPo(diffListR)"
    bind $gPo(diffListR) <Any-KeyRelease> \
	"SynchSelections $gPo(diffListR) $gPo(diffListL)"
    bind $gPo(diffListR) <ButtonRelease-1> \
	"SynchSelections $gPo(diffListR) $gPo(diffListL)"
    bind $gPo(diffListR) <Double-1> \
	"StartGUIDiff $gPo(diffListL) $gPo(diffListR)"
    bind $gPo(diffListR) <Button-3> \
	"OpenMainContextMenu $gPo(diffListR) $gPo(diffListL) %X %Y"

    bind . <Control-a> "SelectAll %X %Y"
    bind . <Key-e> {StartEditor [winfo containing %X %Y] 0}
    bind . <Key-E> {StartEditor [winfo containing %X %Y] 1}
    bind . <Key-h> {StartHexEditor [winfo containing %X %Y] 0}
    bind . <Key-H> {StartHexEditor [winfo containing %X %Y] 1}
    if { $tcl_platform(platform) eq "windows" } {
        bind . <Key-o> {StartAssoc [winfo containing %X %Y] 0}
        bind . <Key-O> {StartAssoc [winfo containing %X %Y] 1}
	bind $gPo(diffListL) <App> \
	    "OpenMainContextMenu $gPo(diffListL) $gPo(diffListR) %X %Y"
	bind $gPo(diffListR) <App> \
	    "OpenMainContextMenu $gPo(diffListR) $gPo(diffListL) %X %Y"
    }
    bind . <Key-d>  "StartGUIDiff $gPo(diffListL) $gPo(diffListR)"
    bind . <Key-D>  "StartGUIDiff $gPo(diffListL) $gPo(diffListR) 1"
    bind . <Key-i>  {ShowFileInfoWin [winfo containing %X %Y]}
    bind . <Key-I>  {ShowFileInfoWin [winfo containing %X %Y] 1}
    bind . <Key-r>  {AskRenameFile [winfo containing %X %Y] %X %Y}
    bind . <Key-F2> {AskRenameFile [winfo containing %X %Y] %X %Y}

    bind $gPo(leftList)  <Delete> {DeleteLeft}
    bind $gPo(rightList) <Delete> {DeleteRight}
    bind $gPo(diffListL) <Delete> {DeleteLeftDiff}
    bind $gPo(diffListR) <Delete> {DeleteRightDiff}
    bind $gPo(diffListL) <Shift-Delete> {DeleteBothDiff}
    bind $gPo(diffListR) <Shift-Delete> {DeleteBothDiff}

    bind $gPo(leftList)  <Key-c> {CopyLeftToRight}
    bind $gPo(rightList) <Key-c> {CopyRightToLeft}
    bind $gPo(diffListL) <Key-c> {CopyLeftDiffToRightDiff}
    bind $gPo(diffListR) <Key-c> {CopyRightDiffToLeftDiff}

    bind $gPo(leftList)  <Key-m> {MoveLeftToRight}
    bind $gPo(rightList) <Key-m> {MoveRightToLeft}
    bind $gPo(diffListL) <Key-m> {MoveLeftDiffToRightDiff}
    bind $gPo(diffListR) <Key-m> {MoveRightDiffToLeftDiff}

    bind . <Key-1> {SetDiffFile [winfo containing %X %Y] 1}
    bind . <Key-2> {SetDiffFile [winfo containing %X %Y] 2}

    # Add new toolbar group and associated buttons.
    set btnFrame [::poToolbar::AddGroup .fr.toolfr]

    ::poToolbar::AddButton $btnFrame.chooseLeft [::poBmpData::openleft] \
			   {ChooseDirs 1} "Select left directory (Ctrl+L)" \
			   -activebackground white
    ::poToolbar::AddButton $btnFrame.chooseRight [::poBmpData::openright] \
			   {ChooseDirs 2} "Select right directory (Ctrl+R)" \
			   -activebackground white
    ::poToolbar::AddButton $btnFrame.diff [::poBmpData::diff] TimeDiffDir \
			   "Diff directories (F5)" \
			   -activebackground white
    ::poToolbar::AddButton $btnFrame.halt [::poBmpData::halt] StopScan \
			   "Stop current diff job (Esc)" -activebackground red

    # Add new toolbar group and associated buttons.
    set btnFrame [::poToolbar::AddGroup .fr.toolfr]
    ::poToolbar::AddButton $btnFrame.search [::poBmpData::search] \
    			   ShowReplaceWin "Show search/replace window (F3)" \
			   -activebackground white

    # Add new toolbar group and associated buttons.
    set btnFrame [::poToolbar::AddGroup .fr.toolfr]

    ::poToolbar::AddButton $btnFrame.errorLog [::poBmpData::logerror] \
    			   ShowErrorLog "Show error log (F6)" \
			   -activebackground white
    ::poToolbar::AddButton $btnFrame.ignoreLog [::poBmpData::logignore] \
    			   ShowIgnoreLog "Show log of ignored files (F7)" \
			   -activebackground white
    ::poToolbar::AddButton $btnFrame.identLog [::poBmpData::logident] \
    			   ShowIdentLog "Show log of identical files (F8)" \
			   -activebackground white

    label .fr.infofr.label -text "HiHi" -anchor w
    pack  .fr.infofr.label -side top -fill x
    UpdateFileCount
    WriteInfoStr $gPo(initStr)
}

proc AskRenameFile { listboxId x y } {
    global gPo errorInfo
    ...
    set retName   [::poWin::EntryBox $shortName $x $y]

    if { $retName eq "" } {
	WriteInfoStr "No file name specified or empty file name. Not renamed."
	return
    }
    ...
    if { $catchVal } {
        lappend gPo(errorLog) [lindex [split "$errorInfo" "\n"] 0]
        incr gPo(readErrors)
	ShowErrorLog
        return
    } 
    ...
    # OPA TODO Not correct with tablelist
    $listboxId insert $curInd [file nativename $newName]
    $listboxId selection set $curInd
    WriteInfoStr "Please rediff manually to update lists."
}

proc DeleteLeft {} {
    global gPo

    ManipFiles $gPo(leftList) $gPo(rightList) \
               $gPo(dir1) $gPo(dir2) 2 ""
}
...
proc DeleteRight {} {
    ...
    ManipFiles $gPo(rightList) $gPo(leftList) \
               $gPo(dir2) $gPo(dir1) 2 ""
}
...
proc DeleteLeftIde {} {
    ...
    ManipFiles $gPo(identListL) $gPo(identListR) \
               $gPo(dir1) $gPo(dir2) 2 $gPo(rightList)
}

proc DeleteRightIde {} {
    global gPo

    ManipFiles $gPo(identListR) $gPo(identListL) \
               $gPo(dir2) $gPo(dir1) 2 $gPo(leftList)
}
...
proc DeleteLeftIgn {} {
    ...
    ManipFiles $gPo(ignListL) $gPo(ignListR) \
               $gPo(dir1) $gPo(dir2) 2 ""
}
...
proc DeleteRightIgn {} {
    ...
    ManipFiles $gPo(ignListR) $gPo(ignListL) \
               $gPo(dir2) $gPo(dir1) 2 ""
}
...
proc DeleteLeftDiff {} {
    ...
    ManipFiles $gPo(diffListL) $gPo(diffListR) \
               $gPo(dir1) $gPo(dir2) 2 $gPo(rightList)
}
...
proc DeleteRightDiff {} {
    ...
    ManipFiles $gPo(diffListR) $gPo(diffListL) \
               $gPo(dir2) $gPo(dir1) 2 $gPo(leftList)
}
...
proc MoveLeftIdeToRightIde {} {
    ...
    ManipFiles $gPo(identListL) $gPo(identListR) \
               $gPo(dir1) $gPo(dir2) 1 $gPo(rightList)
}
...
proc MoveRightIdeToLeftIde {} {
    ...
    ManipFiles $gPo(identListR) $gPo(identListL) \
               $gPo(dir2) $gPo(dir1) 1 $gPo(leftList)
}
...
proc ManipFiles { fromListbox toListbox fromRoot toRoot opCode diffListbox } {
    ...
    set gPo(stopScan) 0
    set dirSep "/"
    set indList [lsort -integer -decreasing [$fromListbox curselection]]
    set numSel [llength $indList]
    set selList {}
    foreach ind $indList {
	lappend selList [$fromListbox get $ind]
    }
    if { $numSel == 1 } {
    ...
    if { $opCode == $opDel && $gPo(deleteConfirm) } {
        set doIt [::poWin::CreateListConfirmWin $selList \
	            "Confirmation" "Delete $fileStr ?"] 
	if { ! $doIt } {
	    return
	}
    } elseif { $opCode == $opCopy && $gPo(copyConfirm) } {
        set doIt [::poWin::CreateListConfirmWin $selList \
	          "Confirmation" "Copy $fileStr to: $toRoot ?"]
	if { ! $doIt } {
	    return
	}
    } elseif { $opCode == $opMove && $gPo(moveConfirm) } {
        set doIt [::poWin::CreateListConfirmWin $selList \
	          "Confirmation" "Move $fileStr to: $toRoot ?"]
	if { ! $doIt } {
	    return
	}
    }
    ...
    foreach ind $indList {
	set fileName1 [AbsPath [$fromListbox get $ind] $fromRoot]
	...
	set native1 [file nativename $fileName1]
	set native2 [file nativename $fileName2]
	if { $opCode == $opMove } {
            WriteInfoStr "$toDo: Moving file $fileName1 to $fileName2"
	    if { $diffListbox eq "" } {
		if { $gPo(relPathes) } {
		    set native2 [AbsToRel $native2 [GetRootDir $toListbox]]
		}
	        $toListbox insert end $native2
	        $toListbox see end
	    } else {
		if { $gPo(relPathes) } {
		    set native2 [AbsToRel $native2 [GetRootDir $diffListbox]]
		}
	        $diffListbox insert end $native2
	        $diffListbox see end
                if { $gPo(markType) } {
                    set markColor [::poFiletype::GetColor $native2]
                    $diffListbox  itemconfigure end -foreground $markColor
                }
	    }
	    file delete $fileName1
	} elseif { $opCode == $opCopy } {
            WriteInfoStr "$toDo: Copying file $fileName1 to $fileName2"
	    if { (( [info exists gPo(identListL)] && \
	            [string equal $fromListbox $gPo(identListL)] ) || 
	          ( [info exists gPo(identListR)] && \
	            [string equal $toListbox $gPo(identListR)] )) || 
	         (( [info exists gPo(identListR)] && \
	            [string equal $fromListbox $gPo(identListR)] ) || 
	          ( [info exists gPo(identListL)] && \
	            [string equal $toListbox $gPo(identListL)] )) } {
	    } else {
		AddToIdentLog $native1 $native2
	    }
	} elseif { $opCode == $opDel } {
            WriteInfoStr "$toDo: Deleting file $fileName1"
	    file delete $fileName1
	    $fromListbox delete $ind
	    if { $diffListbox ne "" } {
	        $toListbox delete $ind
...
proc SetDir { dir num } {
    ...
    set gPo(dir$num) $dir
    if { $num == 1 } {
        WriteInfoStr "Directory $dir set as left directory"
    } else {
        WriteInfoStr "Directory $dir set as right directory"
    }
    InvalidateCache
    UpdateMainTitle
}
...
proc ShowErrorLog {} {
    ...
    frame $tw.fr0
    set listboxId [::poWin::CreateScrolledListbox $tw.fr0 "" \
		   -width 50 -selectmode extended \
		   -exportselection false -bg white]
    if { [llength $gPo(errorLog)] == 0 } {
	$listboxId insert end "No errors encountered"
    } else {
	foreach err $gPo(errorLog) {
    ...
    # Create OK button
    frame $tw.fr1
    button $tw.fr1.b -text "OK" -command "destroy $tw"
    grid $tw.fr0 -row 0 -column 0 -sticky nswe
    grid $tw.fr1 -row 1 -column 0 -sticky nswe
    grid rowconfigure $tw 0 -weight 1
    grid columnconfigure $tw 0 -weight 1
    pack $tw.fr1.b -side left -fill x -expand 1
...
proc DestroyIgnoreLog { tw } {
    ...
    if { [info exists gPo(ignListL)] } {
	unset gPo(ignListL)
    }
    if { [info exists gPo(ignListR)] } {
	unset gPo(ignListR)
    }
    destroy $tw
}
...
proc ShowIgnoreLog {} {
    ...
    set tw .poDiff:IgnoreLog

    catch { DestroyIgnoreLog $tw } 

    toplevel $tw
    wm title $tw "Ignored files"
    wm resizable $tw true true

    frame $tw.frL
    frame $tw.frR
    set leftListbox [::poWin::CreateScrolledListbox $tw.frL "Left" \
		     -width 50 -selectmode extended \
		     -exportselection false -bg white]
    set rightListbox [::poWin::CreateScrolledListbox $tw.frR "Right" \
		     -width 50 -selectmode extended \
		     -exportselection false -bg white]
    bind $leftListbox  <1> "+focus $leftListbox"
    bind $rightListbox <1> "+focus $rightListbox"

    set gPo(ignListL) $leftListbox
    set gPo(ignListR) $rightListbox

    bind $leftListbox <Button-3> \
	"OpenMainContextMenu $leftListbox \"\" %X %Y"
    bind $rightListbox <Button-3> \
	"OpenMainContextMenu $rightListbox \"\" %X %Y"

    bind $tw <Control-a> "SelectAll %X %Y"
    bind $tw <Key-e> {StartEditor [winfo containing %X %Y] 0}
    bind $tw <Key-E> {StartEditor [winfo containing %X %Y] 1}
    bind $tw <Key-h> {StartHexEditor [winfo containing %X %Y] 0}
    bind $tw <Key-H> {StartHexEditor [winfo containing %X %Y] 1}
    if { $tcl_platform(platform) eq "windows" } {
        bind $tw <Key-o> {StartAssoc [winfo containing %X %Y] 0}
        bind $tw <Key-O> {StartAssoc [winfo containing %X %Y] 1}
        bind $tw <App>   {OpenMainContextMenu [winfo containing %X %Y] "" %X %Y}
    }
    bind $tw <Key-i>  {ShowFileInfoWin [winfo containing %X %Y]}
    bind $tw <Key-I>  {ShowFileInfoWin [winfo containing %X %Y] 1}
    bind $tw <Key-r>  {AskRenameFile   [winfo containing %X %Y] %X %Y}
    bind $tw <Key-F2> {AskRenameFile   [winfo containing %X %Y] %X %Y}

    bind $gPo(ignListL) <Delete> {DeleteLeftIgn}
    bind $gPo(ignListR) <Delete> {DeleteRightIgn}

    bind $gPo(ignListL) <Key-c> {CopyLeftIgnToRightIgn}
    bind $gPo(ignListR) <Key-c> {CopyRightIgnToLeftIgn}

    bind $gPo(ignListL) <Key-m> {MoveLeftIgnToRightIgn}
    bind $gPo(ignListR) <Key-m> {MoveRightIgnToLeftIgn}

    foreach ign $gPo(ignLogDirL) {
	$leftListbox insert end $ign
    }
    foreach ign $gPo(ignLogFileL) {
	$leftListbox insert end $ign
	if { $gPo(markType) } {
	    set markColor [::poFiletype::GetColor $ign]
	    $leftListbox  itemconfigure end -foreground $markColor
	}
    }
    foreach ign $gPo(ignLogDirR) {
	$rightListbox insert end $ign
    }
    foreach ign $gPo(ignLogFileR) {
	$rightListbox insert end $ign
	if { $gPo(markType) } {
	    set markColor [::poFiletype::GetColor $ign]
	    $rightListbox itemconfigure end -foreground $markColor
	}
    }

    # Create OK button
    frame $tw.fr1
    button $tw.fr1.b -text "OK" -command "DestroyIgnoreLog $tw"
    grid $tw.frL -row 0 -column 0 -sticky nswe
    grid $tw.frR -row 0 -column 1 -sticky nswe
    grid $tw.fr1 -row 1 -column 0 -sticky nswe -columnspan 2
    grid rowconfigure $tw 0 -weight 1
    grid columnconfigure $tw 0 -weight 1
    grid columnconfigure $tw 1 -weight 1
    pack $tw.fr1.b -side left -fill x -expand 1

    bind $tw <Escape> "DestroyIgnoreLog $tw"
    bind $tw <Return> "DestroyIgnoreLog $tw"
    focus $tw
}
...
proc DestroyIdentLog { tw } {
    ...
    if { [info exists gPo(identListL)] } {
	unset gPo(identListL)
    }
    if { [info exists gPo(identListR)] } {
	unset gPo(identListR)
    }
    destroy $tw
}
...
proc ShowIdentLog {} {
    ...
    frame $tw.fr0
    set listboxList [::poWin::CreateSyncListbox $tw.fr0 "Left" "Right" \
		   -width 50 -selectmode extended \
		   -exportselection false -bg white]
    set leftListbox  [lindex $listboxList 0]
    set rightListbox [lindex $listboxList 1]
    set gPo(identListL) $leftListbox
    set gPo(identListR) $rightListbox

    bind $leftListbox <1>  "+focus $leftListbox"
    bind $rightListbox <1> "+focus $rightListbox"
    bind $leftListbox <Any-KeyRelease> \
	"SynchSelections $leftListbox $rightListbox"
    bind $rightListbox <Any-KeyRelease> \
	"SynchSelections $rightListbox $leftListbox"
    bind $leftListbox <ButtonRelease-1> \
	"SynchSelections $leftListbox $rightListbox"
    bind $rightListbox <ButtonRelease-1> \
	"SynchSelections $rightListbox $leftListbox"
    bind $leftListbox <Button-3> \
	"OpenMainContextMenu $leftListbox $rightListbox %X %Y"
    bind $rightListbox <Button-3> \
	"OpenMainContextMenu $rightListbox $leftListbox %X %Y"
    ...
    bind $gPo(identListL) <Delete> {DeleteLeftIde}
    bind $gPo(identListR) <Delete> {DeleteRightIde}

    bind $gPo(identListL) <Key-c> {CopyLeftIdeToRightIde}
    bind $gPo(identListR) <Key-c> {CopyRightIdeToLeftIde}

    bind $gPo(identListL) <Key-m> {MoveLeftIdeToRightIde}
    bind $gPo(identListR) <Key-m> {MoveRightIdeToLeftIde}

    foreach ident $gPo(identLog) {
	$leftListbox  insert end [lindex $ident 0]
	$rightListbox insert end [lindex $ident 1]
	if { $gPo(markNewer) } {
	    set markLeft  [lindex $ident 2]
	    set markRight [lindex $ident 3]
	    if { $markLeft } {
		$leftListbox itemconf end -background       $gPo(fileMarkColor)
		$leftListbox itemconf end -selectforeground $gPo(fileMarkColor)
	    }
	    if { $markRight } {
		$rightListbox itemconf end -background       $gPo(fileMarkColor)
		$rightListbox itemconf end -selectforeground $gPo(fileMarkColor)
	    }
	}
	if { $gPo(markType) } {
	    set leftMarkColor  [::poFiletype::GetColor [lindex $ident 0]]
	    set rightMarkColor [::poFiletype::GetColor [lindex $ident 1]]
	    $leftListbox  itemconfigure end -foreground $leftMarkColor
	    $rightListbox itemconfigure end -foreground $rightMarkColor
	}
    }

    # Create OK button
    frame $tw.fr1
    button $tw.fr1.b -text "OK" -command "DestroyIdentLog $tw"
    grid $tw.fr0 -row 0 -column 0 -sticky nswe
    grid $tw.fr1 -row 1 -column 0 -sticky nswe
    grid rowconfigure $tw 0 -weight 1
    grid columnconfigure $tw 0 -weight 1
    ...
    bind $tw <Escape> "DestroyIdentLog $tw"
    bind $tw <Return> "DestroyIdentLog $tw"
    focus $tw
}

proc SetDiffFile { listId side } {
    global gPo

    if { $listId eq "" || \
         [winfo class $listId] ne "Listbox" } {
    	return
    }
    if { [winfo class $listId] eq "Listbox" } {
        set indList [$listId curselection]
        if { [llength $indList] > 0 } {
            set ind [lindex $indList 0]
            set fileName [$listId get $ind]
            if { $side == 1 } {
                set gPo(leftFile)  [AbsPath $fileName $gPo(dir1)]
            } else {
                set gPo(rightFile) [AbsPath $fileName $gPo(dir2)]
            }
        }
    }
    WriteInfoStr "Diff set to: \"$gPo(leftFile)\" vs. \"$gPo(rightFile)\""
}
...
proc ConvertFile { masterList mode } {
    ...
    if { $gPo(convConfirm) } {
        ...
        set doIt [::poWin::CreateListConfirmWin $selItemList \
	          "Confirmation" "Convert $fileStr to: $modeStr ?"]
	if { ! $doIt } {
	    return
	}
    }
...
proc DisplayFileInfo { listId infoFr } {
    set indList [$listId curselection]
    if { [llength $indList] == 0 } {
        return
    }
    set ind [lindex $indList end]
    # Convert native name back to Unix notation
    set fileName [AbsPath [$listId get $ind] [GetRootDir $listId]]
    set attrList [::poMisc::FileInfo $fileName]
    set row 0
    foreach listEntry $attrList {
        $infoFr.key_$row configure -text [format "%s:" [lindex $listEntry 0]]
        $infoFr.val_$row configure -text [lindex $listEntry 1]
        incr row
    }
}

proc DisplayTwoFileInfo { leftId rightId leftInfoFr rightInfoFr } {
    DisplayFileInfo $leftId  $leftInfoFr
    DisplayFileInfo $rightId $rightInfoFr
}

proc ShowFileInfoWin { masterList { serialize 0 } } {
    global gPo

    if { $masterList eq "" || \
         ([winfo class $masterList] ne "Listbox" && \
         [winfo class $masterList] ne "Tablelist") } {
    	return
    }
    if { $masterList eq $gPo(diffListL) || \
         $masterList eq $gPo(diffListR) } {
	ShowTwoFileInfoWin $gPo(diffListL) $gPo(diffListR) $serialize
    } elseif { ([info exists gPo(identListL)] && \
                $masterList eq $gPo(identListL)) || \
               ([info exists gPo(identListR)] && \
                $masterList eq $gPo(identListR)) } {
	ShowTwoFileInfoWin $gPo(identListL) $gPo(identListR) $serialize
    } elseif { [winfo class $masterList] eq "Listbox" || \
               [winfo class $masterList] eq "Tablelist" } {
...
proc GetListSelectionIndices { listId } {
    if { $listId eq "" || \
         ([winfo class $listId] ne "Listbox" && \
          [winfo class $listId] ne "Tablelist") } {
    	return [list]
    }
...
proc GetListSelection { listId { columnNum 0 } } {
    ...
    if { [winfo class $listId] eq "Tablelist" } {
        foreach ind $indList {
            if { $columnNum < 0 } {
...
proc ShowOneFileInfoWin { listId { serialize 0 } } {
	...
	incr count
	if { $serialize || $count > $gPo(maxShowWin) } {
            set retVal [tk_messageBox \
              -title "Confirmation" \
	...
	set tw [::poWin::CreateOneFileInfoWin [file tail $fileName] $attrList]
	lappend gPo(infoWinList) $tw
    }
}
...
proc ShowTwoFileInfoWin { leftList rightList { serialize 0 } } {
    ...
    set count 0
    set indList1 [$leftList  curselection]
    set indList2 [$rightList curselection]
    foreach ind1 $indList1 ind2 $indList2 {
	set fileName1 [AbsPath [$leftList  get $ind1] [GetRootDir $leftList]]
	set fileName2 [AbsPath [$rightList get $ind2] [GetRootDir $rightList]]
        set attrList1 [::poMisc::FileInfo $fileName1]
        set attrList2 [::poMisc::FileInfo $fileName2]
	...
	set tw [::poWin::CreateTwoFileInfoWin \
	    [file tail $fileName1] [file tail $fileName2] \
	    $attrList1 $attrList2]
	lappend gPo(infoWinList) $tw
    }
}

proc CloseFileInfoWins {} {
    global gPo

    foreach tw $gPo(infoWinList) {
	if { [winfo exists $tw] } {
	    destroy $tw
	}
    }
    unset gPo(infoWinList)
    set gPo(infoWinList) {}
}

proc OpenContextMenu { dir x y } {

    set w .poDiff:contextMenu
    catch { destroy $w }
    menu $w -tearoff false

    $w configure -title [file tail $dir]
    $w add command -label "Set as left directory"  -command "SetDir $dir 1"
    $w add command -label "Set as right directory" -command "SetDir $dir 2"
    tk_popup $w $x $y
}
...
proc OpenMainContextMenu { masterList slaveList x y } {
    ...
    set numSel [llength [$masterList curselection]]
    if { $numSel == 0 } {
	set menuTitle "Nothing selected"
    } else {
    ...
    set mvStr   ""
    set delStr  ""
    if { $gPo(convConfirm) }   { set convStr " ..." }
    if { $gPo(copyConfirm) }   { set cpStr   " ..." }
    if { $gPo(moveConfirm) }   { set mvStr   " ..." }
    if { $gPo(deleteConfirm) } { set delStr  " ..." }

    # Info and Edit are available for all lists. If we have a slave list,
    # we have to show the 2 files info window.
    if { $slaveList eq "" } {
        $w add command -label "Info" -underline 0 \
	     -command "ShowFileInfoWin $masterList"
    } else {
        $w add command -label "Info" -underline 0 \
	    -command "ShowFileInfoWin $masterList"
    }
    $w add command -label "Edit" -underline 0 \
                   -command "StartEditor $masterList 0"
    if { $tcl_platform(platform) eq "windows" } {
        $w add command -label "Open" -underline 0 \
	    -command "StartAssoc $masterList 0"
    }
    $w add command -label "HexDump" -underline 0 \
                   -command "StartHexEditor $masterList 0"
    $w add command -label "Open directory" -underline 1 \
                   -command "StartFileBrowser $masterList"

    $w add cascade -label "Convert to" -menu $w.conv
    menu $w.conv -tearoff 0
    $w.conv add command -label "Unix (LF) $convStr" \
                        -command "ConvertFile $masterList lf"
    $w.conv add command -label "Dos (CRLF) $convStr" \
                        -command "ConvertFile $masterList crlf"
    $w.conv add command -label "Mac (CR) $convStr" \
                        -command "ConvertFile $masterList cr"

    $w add separator
    $w add command -label "Rename ..." -underline 0 \
    		   -command "AskRenameFile $masterList $x $y"

    # Copy and Move are list specific.
    if { $masterList eq $gPo(leftList) } {
	$w add separator
	$w add command -label "Copy to right $cpStr" -underline 0 \
	               -command "CopyLeftToRight"
	$w add command -label "Move to right $mvStr" -underline 0 \
	               -command "MoveLeftToRight"
	$w add separator
	$w add command -label "Delete $delStr" \
		       -command "DeleteLeft" -activebackground "#FC3030"
    } elseif { $masterList eq $gPo(rightList) } {
	$w add separator
	$w add command -label "Copy to left $cpStr" -underline 0 \
	               -command "CopyRightToLeft"
	$w add command -label "Move to left $mvStr" -underline 0 \
	               -command "MoveRightToLeft"
	$w add separator
	$w add command -label "Delete $delStr" \
		       -command "DeleteRight" -activebackground "#FC3030"
    } elseif { $masterList eq $gPo(diffListL) } {
	$w add separator
	$w add command -label "Copy to right $cpStr" -underline 0 \
	               -command "CopyLeftDiffToRightDiff"
	$w add command -label "Move to right $mvStr" -underline 0 \
	               -command "MoveLeftDiffToRightDiff"
	$w add separator
	$w add cascade -label "Delete" -menu $w.del
	menu $w.del  -tearoff 0
	$w.del add command -label "left $delStr" \
			   -command "DeleteLeftDiff" -activebackground "#FC3030"
	$w.del add command -label "both $delStr" \
			   -command "DeleteBothDiff" -activebackground "#FC3030"
    } elseif { $masterList eq $gPo(diffListR) } {
	$w add separator
	$w add command -label "Copy to left $cpStr" -underline 0 \
	               -command "CopyRightDiffToLeftDiff"
	$w add command -label "Move to left $mvStr" -underline 0 \
	               -command "MoveRightDiffToLeftDiff"
	$w add separator
	$w add cascade -label "Delete" -menu $w.del
	menu $w.del  -tearoff 0
	$w.del add command -label "right $delStr" \
			  -command "DeleteRightDiff" -activebackground "#FC3030"
	$w.del add command -label "both $delStr" \
			   -command "DeleteBothDiff" -activebackground "#FC3030"
    } elseif { [info exists gPo(identListL)] && \
               $masterList eq $gPo(identListL) } {
	$w add separator
	$w add command -label "Copy to right $cpStr" -underline 0 \
	               -command "CopyLeftIdeToRightIde"
	$w add command -label "Move to right $mvStr" -underline 0 \
	               -command "MoveLeftIdeToRightIde"
	$w add separator
	$w add cascade -label "Delete" -menu $w.del
	menu $w.del  -tearoff 0
	$w.del add command -label "left $delStr" \
		       -command "DeleteLeftIde" -activebackground "#FC3030"
	$w.del add command -label "both $delStr" \
			   -command "DeleteBothIde" -activebackground "#FC3030"
    } elseif { [info exists gPo(identListR)] && \
	       $masterList eq $gPo(identListR) } {
	$w add separator
	$w add command -label "Copy to left $cpStr" -underline 0 \
	               -command "CopyRightIdeToLeftIde"
	$w add command -label "Move to left $mvStr" -underline 0 \
	               -command "MoveRightIdeToLeftIde"
	$w add separator
	$w add cascade -label "Delete" -menu $w.del
	menu $w.del  -tearoff 0
	$w.del add command -label "right $delStr" \
		       -command "DeleteRightIde" -activebackground "#FC3030"
	$w.del add command -label "both $delStr" \
			   -command "DeleteBothIde" -activebackground "#FC3030"
    } elseif { [info exists gPo(ignListL)] && \
	       $masterList eq $gPo(ignListL) } {
	$w add separator
	$w add command -label "Copy to right $cpStr" -underline 0 \
	               -command "CopyLeftIgnToRightIgn"
	$w add command -label "Move to right $mvStr" -underline 0 \
	               -command "MoveLeftIgnToRightIgn"
	$w add separator
	$w add command -label "Delete $delStr" \
		       -command "DeleteLeftIgn" -activebackground "#FC3030"
    } elseif { [info exists gPo(ignListR)] && \
	       $masterList eq $gPo(ignListR) } {
	$w add separator
	$w add command -label "Copy to left $cpStr" -underline 0 \
	               -command "CopyRightIgnToLeftIgn"
	$w add command -label "Move to left $mvStr" -underline 0 \
	               -command "MoveRightIgnToLeftIgn"
	$w add separator
	$w add command -label "Delete $delStr" \
		       -command "DeleteRightIgn" -activebackground "#FC3030"
    }

    if { ([info exists gPo(identListL)] && \
          [string equal $masterList $gPo(identListL)]) || \
         ([info exists gPo(identListR)] && \
          [string equal $masterList $gPo(identListR)]) } {
	$w add separator
	$w add cascade -label "Diff" -underline 0 -menu $w.diff
	menu $w.diff -tearoff 0
	$w.diff add command -label "all together now" \
		-command "StartGUIDiff $gPo(identListL) $gPo(identListR) 0"
	$w.diff add command -label "one after the other" \
		-command "StartGUIDiff $gPo(identListL) $gPo(identListR) 1"
    }
    if { ([info exists gPo(diffListL)] && \
          [string equal $masterList $gPo(diffListL)]) || \
         ([info exists gPo(diffListR)] && \
          [string equal $masterList $gPo(diffListR)]) } {
	$w add separator
	$w add cascade -label "Diff" -underline 0 -menu $w.diff
	menu $w.diff -tearoff 0
	$w.diff add command -label "all together now" \
		-command "StartGUIDiff $gPo(diffListL) $gPo(diffListR) 0"
	$w.diff add command -label "one after the other" \
		-command "StartGUIDiff $gPo(diffListL) $gPo(diffListR) 1"
    }
    tk_popup $w $x $y
}
...
proc ChooseDirs { { num 1 } } {
    ...
    set dirStr "dir$num"
    set tmpDir [::poTree::GetDir -initialdir $gPo($dirStr) -showfiles 1 \
                                 -title "Select $str directory"]
...
proc DiffDir {} {
    ...
    ::poLog::Info "DiffDir $gPo(dir1) -- $gPo(dir2)"
    if { [string length $gPo(dir1)] == 0 } {
	tk_messageBox -message "No left directory specified." \
		      -type ok -icon warning
	return	
    }
    if { [string length $gPo(dir2)] == 0 } {
	tk_messageBox -message "No right directory specified." \
		      -type ok -icon warning
	return
    }
    set gPo(stopScan)    0
    set gPo(readErrors)  0
    set gPo(ignLogDirL)  {}
    set gPo(ignLogFileL) {}
    set gPo(ignLogDirR)  {}
    set gPo(ignLogFileR) {}
    ...
    . configure -cursor watch
    ClearFileLists
    if { [lsearch -exact $gPo(dirList) $gPo(dir1)] < 0 } {
        set gPo(dirList) [linsert $gPo(dirList) 0 $gPo(dir1)]
    }
    if { [lsearch -exact $gPo(dirList) $gPo(dir2)] < 0 } {
        set gPo(dirList) [linsert $gPo(dirList) 0 $gPo(dir2)]
    }
    # Add this new directories to the recently used menu.
    AddRecentDirs $gPo(leftMenu)  1 true
    AddRecentDirs $gPo(rightMenu) 2 true

    ScanRecursive $gPo(dir1) $gPo(dir2) $gPo(leftList) 0
    if { ! $gPo(stopScan) } {
	ScanRecursive $gPo(dir2) $gPo(dir1) $gPo(rightList) 1
    }
    . configure -cursor top_left_arrow
    if { $gPo(stopScan) } {
        set infoStr "Diffing cancelled"
    } else {
        set infoStr "Diffing finished"
    }
    if { $gPo(readErrors) } {
	set infoStr [format "%s (%d unreadable files or directories)." \
		     $infoStr $gPo(readErrors)]
    }
    WriteInfoStr $infoStr
    UpdateFileCount
    if { $gPo(readErrors) > 0 } {
	ShowErrorLog
    }
...
proc UpdateFileCount {} {
    ...
    set str [format "Left directory only (%d files)" \
            [$gPo(leftList) index end]]
    ::poWin::SetScrolledTitle $gPo(leftList) $str
    set str [format "Right directory only (%d files)" \
            [$gPo(rightList) index end]]
    ::poWin::SetScrolledTitle $gPo(rightList) $str

    set strL [format "Different left (%d files)" \
             [$gPo(diffListL) index end]]
    set strR [format "Different right (%d files)" \
             [$gPo(diffListR) index end]]
    ::poWin::SetSyncTitle $gPo(diffListR) $strL $strR
}
...
proc UpdateMainTitle {} {
    ...
    wm title . [format "%s (%s <--> %s)" $gPo(appName) $dir1 $dir2]
    update
}
...
proc HelpProg { {splashWin ""} } {
    ...
    ::poSoftLogo::ShowLogo "$gPo(appName) Version 0.3" \
...
proc HelpTcl {} {
    ::poSoftLogo::ShowTclLogo 0 Tk combobox tablelist
}

proc FilesAreIdent { f1 f2 } {
    global gPo errorInfo

    set catchVal [catch { \
	  ::poMisc::FileIdent $f1 $f2 $gPo(cmpMode) $gPo(ignEolChar) } retVal ]
    if { $catchVal } {
        lappend gPo(errorLog) [lindex [split "$errorInfo" "\n"] 0]
        incr gPo(readErrors)
        return 0
...
proc HiliteReplaceDir { { onOff 1 } } {
    global gPo

    ::poLog::Info "HiliteReplaceDir $onOff"
    $gPo(leftList)  configure -bg white
    $gPo(rightList) configure -bg white
    $gPo(diffListL) configure -bg white
    $gPo(diffListR) configure -bg white
    if { !$onOff } {
 	return
    }
    if { $gPo(replaceDir) == 1 } {
	$gPo(leftList)  configure -bg $gPo(dirMarkColor)
	$gPo(diffListL) configure -bg $gPo(dirMarkColor)
    } else {
	$gPo(rightList) configure -bg $gPo(dirMarkColor)
	$gPo(diffListR) configure -bg $gPo(dirMarkColor)
    }
}

proc ShowFileCont { tableId textId } {
    global gPo
    global gSearch
    ...
    set recNum [lindex $indList 0]
    set fileName [lindex [$tableId get $recNum] 0]
    set searchDir $gPo(dir$gPo(replaceDir))
    ...
    if { !$isBinary } {
        ...
        set fileCont [read $fp [file size $fileName]]
        close $fp
    } else {
        set fileCont "Binary file"
    }

    ::poWin::SetScrolledTitle $textId $fileName

    $textId delete 1.0 end
    $textId insert end $fileCont

    set gSearch(indices) [list]
    if { $isBinary } {
        SetSearchButtonStates "disabled"
        return
    }

    set markColor $gPo(fileMarkColor)
    set indexIn 1.0
    set countIn 0
    set i       0
    while { 1 } {
        if { $gPo(ignCaseSearch) } {
            set indexOut [$textId search -count countOut -regexp -nocase \
                          $gPo(curSearchPatt) "$indexIn + $countIn chars" end]
        } else {
        ...
        lappend gSearch(indices)  $indexOut
...
proc SetSearchButtonStates { state } {
    ...
    $gSearch(btn,first) configure -state $state
    $gSearch(btn,last)  configure -state $state
    $gSearch(btn,prev)  configure -state $state
    $gSearch(btn,next)  configure -state $state
}

proc ShowResult { ind } {
    global gSearch gPo

    if { ! [info exists gSearch(curIndex)] || \
         [llength $gSearch(indices)] == 0 } {
        return
    }

    set curIndex $gSearch(curIndex)
    $gSearch(textId) tag configure "found$curIndex" \
                     -background $gPo(fileMarkColor)

    if { $ind == -1 || $ind == 1 } {
        set curIndex [expr $curIndex + $ind]
        set curIndex [::poMisc::Max $curIndex 0]
        set curIndex [::poMisc::Min $curIndex \
                                    [expr [llength $gSearch(indices)] -1]]
    } elseif { $ind eq "end" } {
        set curIndex [expr [llength $gSearch(indices)] -1]
    } else {
        set curIndex $ind
    }
    $gSearch(textId) tag configure "found$curIndex" -background "yellow"
    $gSearch(textId) see [lindex $gSearch(indices) $curIndex]

    set gSearch(curIndex) $curIndex

    SetSearchButtonStates "normal"
    if { $curIndex == 0 } {
        $gSearch(btn,first) configure -state disabled
        $gSearch(btn,prev)  configure -state disabled
    } 
    if { $curIndex == [expr [llength $gSearch(indices)] -1] } {
        $gSearch(btn,last) configure -state disabled
        $gSearch(btn,next)  configure -state disabled
    }
}
...
proc ShowReplaceWin {} {
    ...
    set tw .poDiff:ReplaceWin

    HiliteReplaceDir

    set gSearch(tw)       $tw
    set gSearch(numFiles) 0

    if { [winfo exists $tw] } {
        $gSearch(tableId) delete 0 end
        $gSearch(textId)  delete 1.0 end
        ::poWin::Raise $tw
    ...
    wm title $tw "Search window"
    if { $earlyReturn } {
        return
    }

    frame $tw.toolfr1 -relief sunken -borderwidth 1
    frame $tw.toolfr2 -relief sunken -borderwidth 1
    frame $tw.tablefr
    frame $tw.textfr
    grid $tw.toolfr1 -row 0 -column 0 -sticky nswe
    grid $tw.toolfr2 -row 1 -column 0 -sticky nswe
    grid $tw.tablefr -row 2 -column 0 -sticky nswe
    grid $tw.textfr  -row 3 -column 0 -sticky nswe
    grid rowconfigure $tw 3 -weight 1
    grid columnconfigure $tw 0 -weight 1

    set btnFrame [::poToolbar::AddGroup $tw.toolfr1 2]
    set optFrame [::poToolbar::AddGroup $tw.toolfr1 2]
    set cmdFrame [::poToolbar::AddGroup $tw.toolfr1 2]

    set gSearch(btn,first) $btnFrame.first
    set gSearch(btn,last)  $btnFrame.last
    set gSearch(btn,prev)  $btnFrame.prev
    set gSearch(btn,next)  $btnFrame.next

    ::poToolbar::AddButton $gSearch(btn,first) [::poBmpData::top] \
        {ShowResult 0} "Goto to first occurence" -activebackground white
    ::poToolbar::AddButton $gSearch(btn,prev)  [::poBmpData::up] \
        {ShowResult -1} "Goto to previous occurence (b)" -activebackground white
    ::poToolbar::AddButton $gSearch(btn,next)  [::poBmpData::down] \
        {ShowResult 1} "Goto to next occurence (n)" -activebackground white
    ::poToolbar::AddButton $gSearch(btn,last)  [::poBmpData::bottom] \
        {ShowResult end} "Goto to last occurence" -activebackground white

    ::poToolbar::AddRadioButton $optFrame.left [::poBmpData::left] \
        HiliteReplaceDir "Search in left directory" \
        -variable gPo(replaceDir) -value 1 -activebackground white
    ::poToolbar::AddRadioButton $optFrame.right [::poBmpData::right] \
        HiliteReplaceDir "Search in right directory" \
        -variable gPo(replaceDir) -value 2 -activebackground white
    ::poToolbar::AddCheckButton $optFrame.case "" \
        "" "" -text "Ignore case" -variable gPo(ignCaseSearch) \
        -indicatoron 1 -activebackground white
    ::poToolbar::AddCheckButton $optFrame.linenum "" \
        "ShowLineNums" "" -text "Show linenum" -variable gPo(showLineNums) \
        -indicatoron 1 -activebackground white

    ::poToolbar::AddButton $cmdFrame.searchleft [::poBmpData::searchleft] \
        "TimeReplaceDir searchPatt" "Find search pattern (F3)" \
        -activebackground white
    ::poToolbar::AddButton $cmdFrame.searchright [::poBmpData::searchright] \
        "TimeReplaceDir replacePatt" "Find replace pattern" \
        -activebackground white
    ::poToolbar::AddButton $cmdFrame.replace [::poBmpData::rename] \
        "TimeReplaceDir searchPatt replacePatt" \
        "Perform replacement. This operation cannot be undone" \
        -activebackground white

    SetSearchButtonStates "disabled"

    set txtFrame [::poToolbar::AddGroup $tw.toolfr2]
    ::poToolbar::AddLabel $txtFrame.searchLabel "" "Search:" ""
    ::poToolbar::AddEntry $txtFrame.searchPatt gPo(searchPatt) \
        "Search pattern" -width 15
    ::poToolbar::AddLabel $txtFrame.replaceLabel "" "Replace:" ""
    ::poToolbar::AddEntry $txtFrame.replacePatt gPo(replacePatt) \
        "Replace pattern" -width 15
    ::poToolbar::AddLabel $txtFrame.fileMatchLabel "" "File match:" ""
    ::poToolbar::AddEntry $txtFrame.fileMatchPatt gPo(filePatt) \
        "File match pattern" -width 10

    set tableId [::poWin::CreateScrolledTablelist $tw.tablefr "" \
    ...
    set gSearch(tableId) $tableId

    set textId [::poWin::CreateScrolledText $tw.textfr "File name" \
                -width 50 -height 20 -wrap none -exportselection false \
                -font [::poWin::GetFixedFont]]
    set gSearch(textId) $textId

    $textId tag configure linenum -background yellow
    bind $tableId <<ListboxSelect>> "ShowFileCont %W $textId"

    set bodyTag [$tableId bodytag]
    bind $bodyTag <Button-3> \
	[list OpenMainContextMenu $tableId "" %X %Y]

    bind $bodyTag <Control-a> "SelectAll %X %Y"
    bind $bodyTag <Key-e> "StartEditor $tableId 0"
    bind $bodyTag <Key-E> "StartEditor $tableId 1"
    bind $bodyTag <Key-h> "StartHexEditor $tableId 0"
    bind $bodyTag <Key-H> "StartHexEditor $tableId 1"
    bind $bodyTag <Key-p> "StartFileBrowser $tableId"
    if { $tcl_platform(platform) eq "windows" } {
        bind $bodyTag <Key-o> "StartAssoc $tableId 0"
        bind $bodyTag <Key-O> "StartAssoc $tableId 1"
        bind $bodyTag <App>   [list OpenMainContextMenu $tableId "" %X %Y]
    }
    bind $bodyTag <Key-i>  "ShowFileInfoWin $tableId"
    bind $bodyTag <Key-I>  "ShowFileInfoWin $tableId 1"
    bind $bodyTag <Key-r>  "AskRenameFile   $tableId %X %Y"
    bind $bodyTag <Key-F2> "AskRenameFile   $tableId %X %Y"

    bind $bodyTag <Key-b>  "ShowResult -1"
    bind $bodyTag <Key-n>  "ShowResult  1"

    bind $tw <Key-F3> "TimeReplaceDir searchPatt"
    bind $tw <Escape> "StopScan"
    focus $tw
}
...
proc AddToSearchLog { doReplace fileName numHits } {
    ...
    incr gSearch(numFiles)

    if { $doReplace } {
	wm title $gSearch(tw) \
                 "Replacement results ($gSearch(numFiles) files affected)"
    } else {
	wm title $gSearch(tw) \
                 "Search results ($gSearch(numFiles) files affected)"
    }
    ...
    $gSearch(tableId) insert end [list $fileName $numHits]
    if { $gPo(markType) } {
        set markColor [::poFiletype::GetColor $fileName]
        $gSearch(tableId) rowconfigure end -foreground $markColor
    }
}

proc ReplaceDir { searchPatt replacePatt } {
    global gPo

    set replaceDir $gPo(dir$gPo(replaceDir))
    ::poLog::Info "ReplaceDir $searchPatt replacePatt $replaceDir"
    if { [string length $replaceDir] == 0 } {
	tk_messageBox -message "No search/replace directory specified." \
		      -type ok -icon warning
	return
    }
    ...
    FindRecursive $replaceDir $searchPatt $replacePatt
    . configure -cursor top_left_arrow
    if { $doReplace } {
	set infoStr "Replacing "
    } else {
	set infoStr "Searching "
    }
    if { $gPo(stopScan) } {
        append infoStr "cancelled."
    } else {
        append infoStr "finished."
    }
    if { $gPo(readErrors) } {
	set infoStr [format "%s (%d unreadable files or directories)." \
		     $infoStr $gPo(readErrors)]
...
proc FindRecursive { srcDir searchPatt replacePatt } {
    global tcl_platform errorInfo 
    global gPo

    set useHiddenDirs  [expr !$gPo(ignHiddenDirs)]
    set useHiddenFiles [expr !$gPo(ignHiddenFiles)]

    if { ! [file isdirectory $srcDir] } {
	tk_messageBox -message "$srcDir is not a directory" \
		      -type ok -icon warning
    	return
    }
    ...
    if { $retVal } {
	lappend gPo(errorLog) "Could not read directory \"$srcDir\""
	incr gPo(readErrors)
    }
    if { $replacePatt ne "" } {
        set doReplace true
    ...
    if { $doReplace } {
	WriteInfoStr "Replacing \"$searchPatt\" in directory $srcDir ..."
    } else {
	WriteInfoStr "Searching \"$searchPatt\" in directory $srcDir ..."
    }
    update

    # Note: GetDirList returns directory names as absolute pathes, 
    # file names as relative pathnames.
    set dirCont [::poMisc::GetDirList $srcDir 1 1 $useHiddenDirs $useHiddenFiles]
	...
	foreach ignDir $gPo(ignDirList) {
	    if { [eval $matchCmd {$ignDir $dirName}] } {
		set ignore 1
		break
	    }
	}
	...
	FindRecursive $dir $searchPatt $replacePatt
    }

    set filePattList [split $gPo(filePatt)]
	...
	set ignore 1
	foreach matchFile $filePattList {
	    if { [eval $matchCmd {$matchFile $pureFileName}] } {
		set ignore 0
		break
	    }
	}
	...
	# Check if file matches the ignore patterns.
	set ignore 0
	foreach ignFile $gPo(ignFileList) {
	    if { [eval $matchCmd {$ignFile $pureFileName}] } {
		set ignore 1
		break
	    }
	}
	...
	set catchVal [catch {::poMisc::SearchReplaceInFile $srcNative \
                      $searchPatt $replacePatt \
                      $doReplace $gPo(ignCaseSearch) } retVal]
	if { $catchVal } {
	    lappend gPo(errorLog) [lindex [split "$errorInfo" "\n"] 0]
	    incr gPo(readErrors)
	} else {
	    if { $retVal > 0 } {
                AddToSearchLog $doReplace $srcNative $retVal
	    }
	}
	if { $gPo(stopScan) } {
	    return
...
proc ScanRecursive { srcDir dstDir listId onlyExtraFiles } {
    global tcl_platform errorInfo 
    global gPo gCache

    set showHiddenDirs  [expr !$gPo(ignHiddenDirs)]
    set showHiddenFiles [expr !$gPo(ignHiddenFiles)]
    ...
    WriteInfoStr "Diffing directory $srcDir ..."
    update
    set dirCont [::poMisc::GetDirList $srcDir 1 1 \
		 $showHiddenDirs $showHiddenFiles]
    if { $tcl_platform(platform) eq "unix" } {
        set matchCmd "string match"
    } else {
        set matchCmd "string match -nocase"
	...
	if { [string first "~" $dirName] == 0 } {
        ...
        set srcSubDir [file join $srcDir $dirName]
        set dstSubDir [file join $dstDir $dirName]

	# Source entry is a directory.
        # Compare only the pure directory name, not the whole path.
        set pureDirName [file tail $dirName]
	foreach ignDir $gPo(ignDirList) {
	    if { [eval $matchCmd {$ignDir $pureDirName}] } {
		set ignore 1
		if { $listId eq $gPo(leftList) } {
		    AddToIgnLog ignLogDirL $gPo(dir1) \
				[file nativename "$srcSubDir/"] true
		} else {
		    AddToIgnLog ignLogDirR $gPo(dir2) \
				[file nativename "$srcSubDir/"] true
		}
		break
	    }
	...
	if { ![file isdirectory $dstSubDir] } {
	    # Destination directory does not exist.
	} 
	if { $gPo(stopScan) } {
	    return
	}
	ScanRecursive $srcSubDir $dstSubDir $listId $onlyExtraFiles
    }
    foreach fileName $fileList {
	set ignore 0
	if { [string first "~" $fileName] == 0 } {
	...
	# Source entry is a file.
        set srcSubDir [file join $srcDir $fileName]
        set dstSubDir [file join $dstDir $fileName]
        set srcNative [file nativename $srcSubDir]
        set dstNative [file nativename $dstSubDir]
        # Compare only the pure filename, not the path part.
        set pureFileName [file tail $srcSubDir]
	foreach ignFile $gPo(ignFileList) {
	    if { [eval $matchCmd {$ignFile $pureFileName}] } {
		set ignore 1
		if { $listId eq $gPo(leftList) } {
		    AddToIgnLog ignLogFileL $gPo(dir1) "$srcNative" false
		} else {
		    AddToIgnLog ignLogFileR $gPo(dir2) "$srcNative" false
		}
		break
	    }
	}
	...
	if { ![file exists $dstSubDir] } {
	    AddToFileList $listId $srcNative 0 $gPo(markType)
	} elseif { ! $onlyExtraFiles } {
	    set srcDate [file mtime $srcSubDir]
	    set dstDate [file mtime $dstSubDir]
	    set markSrc 0
	    set markDst 0
	    if { $gPo(markNewer) } {
		if { $srcDate < $dstDate } {
	    ...
	    if { ([file size $srcSubDir] != [file size $dstSubDir]) && \
		 (($gPo(cmpMode) == 0) || \
		  ($gPo(cmpMode) == 2) && ($gPo(ignEolChar) == 0)) } {
		AddToFileList $gPo(diffListL) $srcNative $markSrc $gPo(markType)
		AddToFileList $gPo(diffListR) $dstNative $markDst $gPo(markType)
		continue
	    }

	    if { $gPo(useCache) && \
		 [info exists gCache(date,$srcSubDir)] && \
		 [info exists gCache(date,$dstSubDir)] } {
		set srcCacheDate $gCache(date,$srcSubDir)
		set dstCacheDate $gCache(date,$dstSubDir)
		if { $srcDate == $srcCacheDate && \
		     $dstDate == $dstCacheDate } {
		    if { $gCache(what,$srcSubDir) == "ident" } {
			AddToIdentLog $srcNative $dstNative \
				      $markSrc $markDst
		    } else {
			AddToFileList $gPo(diffListL) $srcNative $markSrc $gPo(markType)
			AddToFileList $gPo(diffListR) $dstNative $markDst $gPo(markType)
	    ...
	    if { ! [FilesAreIdent $srcSubDir $dstSubDir] } {
		AddToFileList $gPo(diffListL) $srcNative $markSrc $gPo(markType)
		AddToFileList $gPo(diffListR) $dstNative $markDst $gPo(markType)
		if { $gPo(useCache) } {
		    set gCache(date,$srcSubDir) $srcDate
		    set gCache(date,$dstSubDir) $dstDate
		    set gCache(what,$srcSubDir) diff
		    set gCache(what,$dstSubDir) diff
		}
	    } else {
		AddToIdentLog $srcNative $dstNative $markSrc $markDst
		if { $gPo(useCache) } {
		    set gCache(date,$srcSubDir) $srcDate
		    set gCache(date,$dstSubDir) $dstDate
		    set gCache(what,$srcSubDir) ident
		    set gCache(what,$dstSubDir) ident
		}
	    }
	}
	if { $gPo(immediateUpdate) } {
...
proc ClearFileLists {} {
    ...
    $gPo(leftList)  delete 0 end
    $gPo(rightList) delete 0 end
    $gPo(diffListL)  delete 0 end
    $gPo(diffListR)  delete 0 end
}
...
proc AddToIdentLog { fileName1 fileName2 { mark1 0 } { mark2 0 } } {
    ...
    if { $gPo(relPathes) } {
	set fileName1 [AbsToRel $fileName1 $gPo(dir1)]
	set fileName2 [AbsToRel $fileName2 $gPo(dir2)]
    }
    lappend gPo(identLog) "[list $fileName1 $fileName2 $mark1 $mark2]"
}
...
proc AddToIgnLog { listName rootName fileName isDir } {
    ...
    lappend gPo($listName) "$fileName"
}

proc AddToFileList { listId fileName { markNewEntry 0 } { markFileTypeEntry 0 } } {
    global gPo
    ...
    $listId insert end $fileName
    if { $markNewEntry } {
        $listId itemconfigure end -background       $gPo(fileMarkColor)
        $listId itemconfigure end -selectforeground $gPo(fileMarkColor)
    }
    if { $markFileTypeEntry } {
	set markColor [::poFiletype::GetColor $fileName]
        $listId itemconfigure end -foreground $markColor
    }
    if { $gPo(immediateUpdate) } {
	$listId see end
    }
...
proc WriteInfoStr { str } {
    .fr.infofr.label configure -text $str
}

proc AppendInfoStr { str } {
    set labelStr [.fr.infofr.label cget -text]
    append labelStr $str
    .fr.infofr.label configure -text $labelStr
}
...
proc InvalidateCache {} {
    ...
    if { [info exists gCache] } {
        unset gCache
    }
}

proc ShowCompareSettWin {} {
    global gPo

    set tw .poDiff:CmpSett
    ...
    toplevel $tw
    wm title $tw "Compare settings"
    wm geometry $tw [format "%dx%d+%d+%d" \
		     $gPo(compare,w) $gPo(compare,h) \
		     $gPo(compare,x) $gPo(compare,y)]

    frame $tw.fr
    pack $tw.fr -fill both -expand 1

    frame $tw.fr.dirfr  -relief ridge -borderwidth 1
    frame $tw.fr.filefr -relief ridge -borderwidth 1
    frame $tw.fr.ignfr  -relief ridge -borderwidth 1
    frame $tw.fr.cmpfr  -relief ridge -borderwidth 1
    frame $tw.fr.eolfr  -relief ridge -borderwidth 1
    frame $tw.fr.okfr   -relief ridge -borderwidth 1
    grid $tw.fr.dirfr   -row 0 -column 0 -sticky news -pady 2 -ipady 2
    grid $tw.fr.filefr  -row 1 -column 0 -sticky news -pady 2 -ipady 2
    grid $tw.fr.ignfr   -row 2 -column 0 -sticky news -pady 1
    grid $tw.fr.cmpfr   -row 3 -column 0 -sticky news -pady 1
    grid $tw.fr.eolfr   -row 4 -column 0 -sticky news -pady 1
    grid $tw.fr.okfr    -row 5 -column 0 -sticky news -pady 2 -ipady 2
    grid rowconfigure $tw.fr 0 -weight 1
    grid rowconfigure $tw.fr 1 -weight 1
    grid columnconfigure $tw.fr 0 -weight 1

    set gPo(ignDirText)  [::poWin::CreateScrolledText $tw.fr.dirfr \
		          "Ignore these directories" -bg white -wrap none -height 5]
    set gPo(ignFileText) [::poWin::CreateScrolledText $tw.fr.filefr \
			  "Ignore these files" -bg white -wrap none -height 5]

    bind $gPo(ignDirText) <Any-KeyRelease> "UpdateIgnoreVar $gPo(ignDirText) ignDirList"
    bind $gPo(ignFileText) <Any-KeyRelease> "UpdateIgnoreVar $gPo(ignFileText) ignFileList"
    ...
    set tmpList [list [list gPo(ignDirList)] [list $gPo(ignDirList)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(ignFileList)] [list $gPo(ignFileList)]]
    lappend varList $tmpList

    # Generate check buttons to ignore hidden directories and files.
    checkbutton $tw.fr.ignfr.cbdir  -text "Ignore hidden directories" \
                                    -variable gPo(ignHiddenDirs)
    checkbutton $tw.fr.ignfr.cbfile -text "Ignore hidden files" \
                                    -variable gPo(ignHiddenFiles)
    pack $tw.fr.ignfr.cbdir $tw.fr.ignfr.cbfile -side left \
         -in $tw.fr.ignfr

    set tmpList [list [list gPo(ignHiddenDirs)] [list $gPo(ignHiddenDirs)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(ignHiddenFiles)] [list $gPo(ignHiddenFiles)]]
    lappend varList $tmpList

    # Generate radio buttons to select file compare mode.
    label $tw.fr.cmpfr.l -text "Compare mode: "
    radiobutton $tw.fr.cmpfr.rb1 -text "Size" -value 0 \
                                 -variable gPo(cmpMode) -command InvalidateCache
    radiobutton $tw.fr.cmpfr.rb2 -text "Date" -value 1 \
                                 -variable gPo(cmpMode) -command InvalidateCache
    radiobutton $tw.fr.cmpfr.rb3 -text "Content" -value 2 \
                                 -variable gPo(cmpMode) -command InvalidateCache
    pack $tw.fr.cmpfr.l $tw.fr.cmpfr.rb1 $tw.fr.cmpfr.rb2 $tw.fr.cmpfr.rb3 \
         -side left -in $tw.fr.cmpfr

    set tmpList [list [list gPo(cmpMode)] [list $gPo(cmpMode)]]
    lappend varList $tmpList

    # Generate check button to ignore platform specific EOL characters.
    label $tw.fr.eolfr.l -text "Content submode: "
    checkbutton $tw.fr.eolfr.cb -text "Ignore EOL character" \
                                -variable gPo(ignEolChar)

    pack $tw.fr.eolfr.l $tw.fr.eolfr.cb -side left -in $tw.fr.eolfr

    set tmpList [list [list gPo(ignEolChar)] [list $gPo(ignEolChar)]]
    lappend varList $tmpList


    # Create Cancel and OK buttons
    bind   $tw <KeyPress-Escape> "CancelToplevel $tw $varList"
    button $tw.fr.okfr.b1 -text "Cancel" -command "CancelToplevel $tw $varList"
    button $tw.fr.okfr.b2 -text "OK"     -command "UpdateIgnoreList $tw"
    pack $tw.fr.okfr.b1 $tw.fr.okfr.b2 -side left -fill x -padx 2 -expand 1
    focus $tw
}
...
proc UpdateIgnoreList { tw } {
    ...
    UpdateIgnoreVar $gPo(ignDirText)  ignDirList
    UpdateIgnoreVar $gPo(ignFileText) ignFileList
    OKToplevel $tw
}
...
proc ShowFileTypeSettWin {} {
    ::poFiletype::OpenWin 
}

proc ShowImgTypeSettWin {} {
    ::poImgtype::OpenWin 
}

proc GetMarkColor { buttonId whichMark } {
    global gPo

    set newColor [tk_chooseColor -initialcolor $gPo($whichMark)]
    if { $newColor ne "" } {
	set gPo($whichMark) $newColor
...
proc ShowMiscSettWin {} {
    ...
    checkbutton $tw.fr$row.cb1 -text "Ask before delete" \
                               -variable gPo(deleteConfirm)
    checkbutton $tw.fr$row.cb2 -text "Ask before copy" \
                               -variable gPo(copyConfirm)
    checkbutton $tw.fr$row.cb3 -text "Ask before move" \
                               -variable gPo(moveConfirm)
    checkbutton $tw.fr$row.cb4 -text "Ask before conversion" \
                               -variable gPo(convConfirm)
    pack $tw.fr$row.cb1 $tw.fr$row.cb2 $tw.fr$row.cb3 $tw.fr$row.cb4 \
	 -side top -anchor w -in $tw.fr$row 

    # Part 2: Mode switches
    incr row
    frame $tw.fr$row
    grid $tw.fr$row -row $row -column 1 -sticky nw
    ...
    checkbutton $tw.fr$row.cb2 -text "Start diff at startup" \
                               -variable gPo(diffAtStartup)
    checkbutton $tw.fr$row.cb3 -text "Internal caching" \
                               -variable gPo(useCache)
    pack $tw.fr$row.cb1 $tw.fr$row.cb2 $tw.fr$row.cb3 \
	 -side top -anchor w -in $tw.fr$row 

    # Part 3: Viewing switches
    incr row
    frame $tw.fr$row
    grid $tw.fr$row -row $row -column 1 -sticky nw

    checkbutton $tw.fr$row.cb1 -text "Relative pathnames" \
                               -variable gPo(relPathes)
    checkbutton $tw.fr$row.cb2 -text "Immediate update" \
                               -variable gPo(immediateUpdate)
    checkbutton $tw.fr$row.cb3 -text "Mark newer files" \
                               -variable gPo(markNewer)
    checkbutton $tw.fr$row.cb4 -text "Mark files by tpye" \
                               -variable gPo(markType)
    pack $tw.fr$row.cb1 $tw.fr$row.cb2 $tw.fr$row.cb3 $tw.fr$row.cb4 \
	 -side top -anchor w -in $tw.fr$row 

    # Part 4: Color settings
    incr row
    frame $tw.fr$row
    grid $tw.fr$row -row $row -column 1 -sticky news

    button $tw.fr$row.b1  -text "File marker ..." -bg $gPo(fileMarkColor) \
                          -command "GetMarkColor $tw.fr$row.b1 fileMarkColor"
    button $tw.fr$row.b2  -text "Dir marker ..." -bg $gPo(dirMarkColor) \
                          -command "GetMarkColor $tw.fr$row.b2 dirMarkColor"
    pack $tw.fr$row.b1 $tw.fr$row.b2 \
	 -side top -fill both -expand 1 -padx 3 -pady 1 -in $tw.fr$row 

    # Part 5: Browse menu entries
    incr row
    frame $tw.fr$row
    grid $tw.fr$row -row $row -column 1 -sticky news

    entry $tw.fr$row.e -textvariable gPo(dirListLen) -width 3
    button $tw.fr$row.b -text "Clear list" \
           -command "ClearRecentDirList $gPo(leftMenu) $gPo(rightMenu)"
    ...
    entry $tw.fr$row.e -textvariable gPo(sessionListLen) -width 3
    button $tw.fr$row.b -text "Clear list" \
           -command "ClearRecentSessionList $gPo(sessionMenu)"
    ::poToolhelp::AddBinding $tw.fr$row.b "This operation can not be canceled"
    pack  $tw.fr$row.e -side left -pady 2 -padx 5
    pack  $tw.fr$row.b -side left -pady 2 -fill x -expand 1

    set tmpList [list [list gPo(deleteConfirm)] [list $gPo(deleteConfirm)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(copyConfirm)]   [list $gPo(copyConfirm)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(moveConfirm)]   [list $gPo(moveConfirm)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(convConfirm)]   [list $gPo(convConfirm)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(useCache)] [list $gPo(useCache)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(relPathes)] [list $gPo(relPathes)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(showSplash)] [list $gPo(showSplash)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(diffAtStartup)] [list $gPo(diffAtStartup)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(immediateUpdate)] [list $gPo(immediateUpdate)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(markNewer)] [list $gPo(markNewer)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(markType)] [list $gPo(markType)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(fileMarkColor)] [list $gPo(fileMarkColor)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(dirMarkColor)] [list $gPo(dirMarkColor)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(dirListLen)] [list $gPo(dirListLen)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(sessionListLen)] [list $gPo(sessionListLen)]]
    lappend varList $tmpList
    ...
    bind  $tw <KeyPress-Escape> "CancelToplevel $tw $varList"
    bind  $tw <KeyPress-Return> "OKToplevel $tw"
    button $tw.fr$row.b1 -text "Cancel" -command "CancelToplevel $tw $varList"
    button $tw.fr$row.b2 -text "OK"     -command "OKToplevel $tw" \
			 -default active
    pack $tw.fr$row.b1 $tw.fr$row.b2 -side left -fill x -padx 2 -expand 1
    focus $tw
}
...
proc LoadSettings {} {
    global gPo tcl_platform

    set gPo(settChanged) false		; # Flag indicating settings changes

    # Init global variables not stored in the cfg file.
    set gPo(identListL) "unknown"	; # Just initialize with some value
    set gPo(identListR) "unknown"	; # Just initialize with some value
    set gPo(ignListL)   "unknown"	; # Just initialize with some value
    set gPo(ignListR)   "unknown"	; # Just initialize with some value
    set gPo(ignLogDirL)  {}
    set gPo(ignLogFileL) {}
    set gPo(ignLogDirR)  {}
    set gPo(ignLogFileR) {}
    ...
    set gPo(compare,x)       100
    set gPo(compare,y)       100
    set gPo(compare,w)       300
    set gPo(compare,h)       400
    set gPo(miscSett,x)      110
    set gPo(miscSett,y)      110
    set gPo(replaceSett,x)   120
    set gPo(replaceSett,y)   120
    set gPo(main,w)          500
    set gPo(main,h)          400
    set gPo(main,x)          10
    set gPo(main,y)          30
    ...
    set gPo(sashY1)          200
    set gPo(ignDirList)      {}
    set gPo(ignFileList)     {*.o *.obj}
    set gPo(dirList)         {}
    set gPo(dirListLen)      10
    set gPo(sessionListLen)  10
    set gPo(dir1)            {}
    set gPo(dir2)            {}
    set gPo(deleteConfirm)   1
    set gPo(copyConfirm)     1
    set gPo(moveConfirm)     1
    set gPo(convConfirm)     1
    set gPo(useCache)        1
    set gPo(cmpMode)         2
    set gPo(ignEolChar)      0
    set gPo(ignHiddenDirs)   0
    set gPo(ignHiddenFiles)  0
    set gPo(maxShowWin)      5
    set gPo(relPathes)       0
    set gPo(immediateUpdate) 1
    set gPo(markNewer)       1
    set gPo(markType)        1
    set gPo(fileMarkColor)   "green"
    set gPo(dirMarkColor)    "lightyellow"
    set gPo(showSplash)      1
    set gPo(diffAtStartup)   0
    set gPo(autosaveOnExit)  1

    set gPo(searchPatt)      ""
    set gPo(replacePatt)     ""
    set gPo(filePatt)        "*"
    set gPo(replaceDir)      2		; # Default replace dir is right
    set gPo(ignCaseSearch)   0		; # Ignore case by default
...
proc SaveSettings {} {
    ...
    set retVal [catch {open $cfgFile w} fp]
    if { $retVal == 0 } {
	puts $fp "set gPo(main,x)  \{[::poMisc::Max [winfo x .] 0]\}"
	puts $fp "set gPo(main,y)  \{[::poMisc::Max [winfo y .] 0]\}"
	puts $fp "set gPo(main,w)  \{[winfo width .]\}"
	puts $fp "set gPo(main,h)  \{[winfo height .]\}"

        set gPo(sashY0) [lindex [$gPo(paneWin) sash coord 0] 1]
        set gPo(sashY1) [lindex [$gPo(paneWin) sash coord 1] 1]
        puts $fp "set gPo(sashY0)   \{$gPo(sashY0)\}"
        puts $fp "set gPo(sashY1)   \{$gPo(sashY1)\}"

	if { [winfo exists .poDiff:CmpSett] } {
	    puts $fp "set gPo(compare,x) \{[winfo x .poDiff:CmpSett]\}"
	    puts $fp "set gPo(compare,y) \{[winfo y .poDiff:CmpSett]\}"
	    puts $fp "set gPo(compare,w) \{[winfo width .poDiff:CmpSett]\}"
	    puts $fp "set gPo(compare,h) \{[winfo height .poDiff:CmpSett]\}"
	} else {
	    puts $fp "set gPo(compare,x)  \{$gPo(compare,x)\}"
	    puts $fp "set gPo(compare,y)  \{$gPo(compare,y)\}"
	    puts $fp "set gPo(compare,w)  \{$gPo(compare,w)\}"
	    puts $fp "set gPo(compare,h)  \{$gPo(compare,h)\}"
	}

	if { [winfo exists .poDiff:sett] } {
	    puts $fp "set gPo(miscSett,x)  \{[winfo x .poDiff:sett]\}"
	    puts $fp "set gPo(miscSett,y)  \{[winfo y .poDiff:sett]\}"
	} else {
	    puts $fp "set gPo(miscSett,x)  \{$gPo(miscSett,x)\}"
	    puts $fp "set gPo(miscSett,y)  \{$gPo(miscSett,y)\}"
	}

	if { [winfo exists .poDiff:replace] } {
	    puts $fp "set gPo(replaceSett,x)  \{[winfo x .poDiff:replace]\}"
	    puts $fp "set gPo(replaceSett,y)  \{[winfo y .poDiff:replace]\}"
	} else {
	    puts $fp "set gPo(replaceSett,x)  \{$gPo(replaceSett,x)\}"
	    puts $fp "set gPo(replaceSett,y)  \{$gPo(replaceSett,y)\}"
	}

	puts $fp "set gPo(ignDirList)  \{$gPo(ignDirList)\}"
	puts $fp "set gPo(ignFileList) \{$gPo(ignFileList)\}"

	puts $fp "set gPo(dir1)         \{$gPo(dir1)\}"
	puts $fp "set gPo(dir2)         \{$gPo(dir2)\}"
	puts $fp "set gPo(dirListLen)   \{$gPo(dirListLen)\}"
	puts $fp "set gPo(dirList)      \{[lrange $gPo(dirList) 0 \
					  [expr $gPo(dirListLen) -1]]\}"

	puts $fp "set gPo(maxShowWin)      \{$gPo(maxShowWin)\}"
	puts $fp "set gPo(deleteConfirm)   \{$gPo(deleteConfirm)\}"
	puts $fp "set gPo(copyConfirm)     \{$gPo(copyConfirm)\}"
	puts $fp "set gPo(moveConfirm)     \{$gPo(moveConfirm)\}"
	puts $fp "set gPo(convConfirm)     \{$gPo(convConfirm)\}"
	puts $fp "set gPo(useCache)        \{$gPo(useCache)\}"
	puts $fp "set gPo(relPathes)       \{$gPo(relPathes)\}"
	puts $fp "set gPo(cmpMode)         \{$gPo(cmpMode)\}"
	puts $fp "set gPo(ignEolChar)      \{$gPo(ignEolChar)\}"
	puts $fp "set gPo(ignHiddenDirs)   \{$gPo(ignHiddenDirs)\}"
	puts $fp "set gPo(ignHiddenFiles)  \{$gPo(ignHiddenFiles)\}"
	puts $fp "set gPo(immediateUpdate) \{$gPo(immediateUpdate)\}"
	puts $fp "set gPo(markNewer)       \{$gPo(markNewer)\}"
	puts $fp "set gPo(markType)        \{$gPo(markType)\}"
	puts $fp "set gPo(fileMarkColor)   \{$gPo(fileMarkColor)\}"
	puts $fp "set gPo(dirMarkColor)    \{$gPo(dirMarkColor)\}"
	puts $fp "set gPo(showSplash)      \{$gPo(showSplash)\}"
	puts $fp "set gPo(diffAtStartup)   \{$gPo(diffAtStartup)\}"
	puts $fp "set gPo(autosaveOnExit)  \{$gPo(autosaveOnExit)\}"

	puts $fp "set gPo(searchPatt)      \{$gPo(searchPatt)\}"
	puts $fp "set gPo(replacePatt)     \{$gPo(replacePatt)\}"
	puts $fp "set gPo(filePatt)        \{$gPo(filePatt)\}"
	puts $fp "set gPo(replaceDir)      \{$gPo(replaceDir)\}"
	puts $fp "set gPo(ignCaseSearch)   \{$gPo(ignCaseSearch)\}"

	puts $fp "::poLog::ShowConsole   \{[::poLog::GetConsoleMode]\}"
	set levels [::poLog::GetDebugLevels]
	puts $fp "::poLog::SetDebugLevels \{$levels\}"

	puts $fp "set gPo(sessionListLen)   \{$gPo(sessionListLen)\}"
	foreach session [lrange [lsort [array names gPo "session,*,dir1"]] \
		         0 [expr $gPo(sessionListLen) -1]] {
	    set sessionName [lindex [split $session ","] 1]
	    foreach entry [lsort [array names gPo "session,$sessionName,*"]] {
		set quotedSession [::poMisc::QuoteSpaces $entry]
		puts $fp "set gPo($quotedSession) \{$gPo($entry)\}"
	    }
	}
...
set gPo(appName)   "poDiff"	; # Name of main application
set gPo(optCfgDir) ""		; # Default: Read from user's home directory.
set gPo(startDir)  [pwd]	; # Start directory
set gPo(verbose)   false	; # Verbose mode
set gPo(infoWinList) {}		; # Start with empty file info window list
set gPo(leftFile)  ""		; # Left  file for selective diff
set gPo(rightFile) ""		; # Right file for selective diff
set gPo(curSession) "Default"   ; # Default session name
...
if { $argc >= 1 } {
	...
	if { [string compare -length 1 $curParam "-"] == 0 } {
	    if { $curParam eq "-verbose" } {
		set gPo(verbose) true
	    } elseif { $curParam eq "-config" } {
		incr curArg
		...
		set gPo(optCfgDir) [::poMisc::FileSlashName $tmpDir]
	    ...
	    set dir [RelToAbsPath $curParam $gPo(startDir)]
	    if { ! [file isdirectory $dir] } {
		::poLog::Warning "$dir is not a directory."
	    } else {
		set gPo(paramdir$whatDir) $dir
		incr whatDir
	    }
	}
	incr curArg
...
::poToolhelp::Init .poLogoToolhelp "#FFFFB5" black
...
# Check for directories supplied as command line parameters.
# If there are no directories specified on the command line nor in the
# settings file, use the start directory instead.
if { [info exists gPo(paramdir1)] } {
    set gPo(dir1) $gPo(paramdir1)
}
if { $gPo(dir1) eq "" } {
    set gPo(dir1) $gPo(startDir)
}
if { [info exists gPo(paramdir2)] } {
    set gPo(dir2) $gPo(paramdir2)
}
if { $gPo(dir2) eq "" } {
    set gPo(dir2) $gPo(startDir)
}
if { [llength $gPo(dirList)] == 0 } {
    if { $gPo(dir1) eq $gPo(dir2) } {
        set gPo(dirList) [list $gPo(dir1)]
    } else {
        set gPo(dirList) [list $gPo(dir1) $gPo(dir2)]
    }
}

UpdateMainTitle

if { $gPo(diffAtStartup) } {
    TimeDiffDir
}
