...
#
#       Module:   	poBitmap
#       Copyright:      Paul Obermeier 2001-2005
#       Filename:       poBitmap.tcl
#
...
#			Functionality is very similar to the standard bitmap
#			tool available on Unix systems, i.e. left mouse button
#			sets pixel values, right mouse button clears pixel 
#			values.
#			Features not available in "bitmap":
#			Generation of Tcl packages out of a series of bitmaps.
#			Simple Tcl plugin interface for extending functionality.
#			Save bitmaps in several formats (needs Img extension).
#
#       Additional documentation:
#			See http://www.poSoft.de for screenshots and plugin
#			examples.
#
#       Exported functions:
#			poplUpdate
...
# Place all needed packages here. !!!
InitPackages Tk poTcllib poTklib poExtlib Img
...
proc UpdateMainTitle { {bmpChanged 0} } {
    ...
    set gPo(bmpChanged) $bmpChanged
    if { $bmpChanged } {
        set star "*"
    } else {
	set star ""
    }
    set msg [format "%s - %s %s" $gPo(appName) $gBmp(defName) $star] 
    wm title .po $msg
}
...
proc FillPluginMenu {} {
    ...
    set plugMenu $gPo(plugMenu)
    catch { destroy $plugMenu }
    menu $plugMenu -tearoff 0
    if { [file isdirectory $gPo(pluginDir)] } {
	set fileList [glob -nocomplain $gPo(pluginDir)/*.pop]
	set count 1
	if { [llength $fileList] != 0 } {
	    foreach f [lsort $fileList] {
		set shortName [file tail [file root $f]]
		if { $count <= 9 } {
		    bind .po <Control-Key-$count> "ExecPlugin $f"
		    set abbr "Ctrl+$count"
		} else {
		    set abbr ""
		}
		AddMenuCmd $plugMenu $shortName "$abbr" "ExecPlugin $f"
		incr count
	    }
	} else {
	    AddMenuCmd $plugMenu "No plugins available" "" "" -state disabled
	}
    } else {
	AddMenuCmd $plugMenu "No plugin directory" "" "" -state disabled
    }
}
...
proc ShowMainWin { title } {
    ...
    wm geometry .po [format "%dx%d+%d+%d" \
       $gPo(main,w) $gPo(main,h) $gPo(main,x) $gPo(main,y)]
    ...
    frame .po.workfr.prefr  -relief raised -borderwidth 1
    frame .po.workfr.infofr -relief sunken -borderwidth 1
 
    grid .po.workfr.prefr  -row 0 -column 0 -sticky nwse
    grid .po.workfr.imgfr  -row 0 -column 1 -sticky nwse
    grid .po.workfr.infofr -row 1 -column 0 -sticky nwse -columnspan 2
    grid rowconfigure .po.workfr 0 -weight 1
    grid columnconfigure .po.workfr 1 -weight 1
    ...
    $hMenu add cascade -menu $hMenu.view -label View     -underline 0
    $hMenu add cascade -menu $hMenu.sett -label Settings -underline 0
    $hMenu add cascade -menu $hMenu.plug -label Plugins  -underline 0
    $hMenu add cascade -menu $hMenu.help -label Help     -underline 0
    ...
    AddMenuCmd $fileMenu "New ..."    "Ctrl+N" "NewBmp"
    AddMenuCmd $fileMenu "Open ..."   "Ctrl+O" "OpenBmpFile"
    AddMenuCmd $fileMenu "Browse ..." "Ctrl+B" "AskBrowseDir"
    ...
    AddMenuCmd $fileMenu "Save ..."   "Ctrl+S" "SaveBmpFile"
    AddMenuCmd $fileMenu "Quit"       "Ctrl+Q" ExitProg

    bind .po <Control-n>  "NewBmp"
    bind .po <Control-o>  "OpenBmpFile"
    bind .po <Control-b>  "AskBrowseDir"
    bind .po <Control-s>  "SaveBmpFile"
    bind .po <Control-q>  ExitProg
    wm protocol .po WM_DELETE_WINDOW "ExitProg"

    set viewMenu  $hMenu.view
    menu $viewMenu -tearoff 0
    AddMenuCmd $viewMenu "Bitmap Info"    "Ctrl+I" BmpInfo
    bind .po <Control-i> BmpInfo

    set editMenu $hMenu.edit
    menu $editMenu -tearoff 0
    AddMenuCmd $editMenu "Undo"              "Ctrl+Z" "Undo"
    AddMenuCmd $editMenu "Redo"              "Ctrl+R" "Redo"
    $editMenu add separator
    AddMenuCmd $editMenu "Copy ..."          "Ctrl+C" "AskCopyImg"
    AddMenuCmd $editMenu "Cut ..."           "Ctrl+X" "AskCutImg"
    AddMenuCmd $editMenu "Paste ..."         "Ctrl+V" "AskPasteImg"
    $editMenu add separator
    AddMenuCmd $editMenu "Clear"             "Ctrl+-" "ClearBmp"
    AddMenuCmd $editMenu "Invert"            "Ctrl+," "InvertBmp"
    AddMenuCmd $editMenu "Flip horizontally" "" "FlipHori"
    AddMenuCmd $editMenu "Flip vertically"   "" "FlipVert"
    AddMenuCmd $editMenu "Shift left"        "Ctrl+H" "ShiftBmp -1  0"
    AddMenuCmd $editMenu "Shift right"       "Ctrl+L" "ShiftBmp  1  0"
    AddMenuCmd $editMenu "Shift up"          "Ctrl+K" "ShiftBmp  0 -1"
    AddMenuCmd $editMenu "Shift down"        "Ctrl+J" "ShiftBmp  0  1"
    bind .po <Control-z>     "Undo"
    bind .po <Control-r>     "Redo"
    bind .po <Control-c>     "AskCopyImg"
    bind .po <Control-x>     "AskCutImg"
    bind .po <Control-v>     "AskPasteImg"
    bind .po <Control-minus> "ClearBmp"
    bind .po <Control-comma> "InvertBmp"
    bind .po <Control-h>     "ShiftBmp -1  0"
    bind .po <Control-l>     "ShiftBmp  1  0"
    bind .po <Control-k>     "ShiftBmp  0 -1"
    bind .po <Control-j>     "ShiftBmp  0  1"
    bind .po <Left>          "poplLeft"
    bind .po <Right>         "poplRight"
    bind .po <Up>            "poplUp"
    bind .po <Down>          "poplDown"
    bind .po <Control-Left>  "poplDraw 0;  poplLeft"
    bind .po <Control-Right> "poplDraw 0;  poplRight"
    bind .po <Control-Up>    "poplDraw 0;  poplUp"
    bind .po <Control-Down>  "poplDraw 0;  poplDown"
    bind .po <Shift-Left>    "poplClear 0; poplLeft"
    bind .po <Shift-Right>   "poplClear 0; poplRight"
    bind .po <Shift-Up>      "poplClear 0; poplUp"
    bind .po <Shift-Down>    "poplClear 0; poplDown"
    bind .po <Control-KeyRelease-Left>  "UpdatePreview; SaveUndoImg"
    bind .po <Control-KeyRelease-Right> "UpdatePreview; SaveUndoImg"
    bind .po <Control-KeyRelease-Up>    "UpdatePreview; SaveUndoImg"
    bind .po <Control-KeyRelease-Down>  "UpdatePreview; SaveUndoImg"
    bind .po <Shift-KeyRelease-Left>    "UpdatePreview; SaveUndoImg"
    bind .po <Shift-KeyRelease-Right>   "UpdatePreview; SaveUndoImg"
    bind .po <Shift-KeyRelease-Up>      "UpdatePreview; SaveUndoImg"
    bind .po <Shift-KeyRelease-Down>    "UpdatePreview; SaveUndoImg"

    set settMenu $hMenu.sett
    menu $settMenu -tearoff 0
    AddMenuCmd $settMenu "Logging ..."  "" ::poLogOpt::OpenWin
    AddMenuCmd $settMenu "Misc ..."     "" ShowMiscSettWin
    AddMenuCmd $settMenu "Colors ..."   "" ShowColorSettWin
    AddMenuCmd $settMenu "Package ..."  "" ShowBmpPackageWin
    ...
    set canvasId [::poWin::CreateScrolledCanvas .po.workfr.imgfr \
		  "" -borderwidth 0 -relief sunken -closeenough 0]
    set gBmp(canvId) $canvasId
    CreateGrid $canvasId $gBmp(xsize) $gBmp(ysize) $gBmp(pixsize)

    $canvasId bind rect <ButtonPress-1> "SetPixel %W %x %y 1"
    $canvasId bind rect <B1-Motion>     "SetPixel %W %x %y 1"
    $canvasId bind rect <ButtonPress-3> "SetPixel %W %x %y 0"
    $canvasId bind rect <B3-Motion>     "SetPixel %W %x %y 0"
    $canvasId bind rect <ButtonRelease-1> "UpdatePreview; SaveUndoImg"
    $canvasId bind rect <ButtonRelease-3> "UpdatePreview; SaveUndoImg"

    # Add new toolbar group and associated buttons.
    set btnFrame [::poToolbar::AddGroup .po.toolfr]

    ::poToolbar::AddButton $btnFrame.new [::poBmpData::newfile] \
	    NewBmp "New bitmap ... (Ctrl+N)" -activebackground white

    ::poToolbar::AddButton $btnFrame.open [::poBmpData::open] \
	    OpenBmpFile "Open bitmap file ... (Ctrl+O)" -activebackground white

    ::poToolbar::AddButton $btnFrame.browse [::poBmpData::browse] \
	    AskBrowseDir "Browse directory for bitmaps ... (Ctrl+B)" \
	    -activebackground white

    ::poToolbar::AddButton $btnFrame.save [::poBmpData::save] \
	    SaveBmpFile "Save bitmap to file ...(Ctrl+S)" \
	    -activebackground white

    # Add new toolbar group and associated buttons.
    set btnFrame [::poToolbar::AddGroup .po.toolfr]

    ::poToolbar::AddButton $btnFrame.undo [::poBmpData::undo] "Undo" \
	    "Undo (Ctrl+Z)" -activebackground white
    ::poToolbar::AddButton $btnFrame.redo [::poBmpData::redo] "Redo" \
	    "Redo (Ctrl+R)" -activebackground white

    # Add new toolbar group and associated buttons.
    set btnFrame [::poToolbar::AddGroup .po.toolfr]

    ::poToolbar::AddButton $btnFrame.copy [::poBmpData::copy] "AskCopyImg" \
	    "Copy bitmap ... (Ctrl+C)" -activebackground white
    ::poToolbar::AddButton $btnFrame.cut [::poBmpData::cut] "AskCutImg" \
	    "Cut bitmap ... (Ctrl+X)" -activebackground white
    ::poToolbar::AddButton $btnFrame.paste [::poBmpData::paste] "AskPasteImg" \
	    "Paste bitmap ... (Ctrl+V)" -activebackground white

    # Add new toolbar group and associated buttons.
    set btnFrame [::poToolbar::AddGroup .po.toolfr]

    ::poToolbar::AddButton $btnFrame.clear [::poBmpData::clear] "ClearBmp" \
	    "Clear bitmap (Ctrl+-)" -activebackground white
    ::poToolbar::AddButton $btnFrame.invert [::poBmpData::invert] "InvertBmp" \
	    "Invert bitmap (Ctrl+,)" -activebackground white
    ::poToolbar::AddButton $btnFrame.fliph [::poBmpData::fliphori] "FlipHori" \
	    "Flip bitmap horizontally" -activebackground white
    ::poToolbar::AddButton $btnFrame.flipv [::poBmpData::flipvert] "FlipVert" \
	    "Flip bitmap vertically" -activebackground white
    ::poToolbar::AddButton $btnFrame.left [::poBmpData::left]  "ShiftBmp -1 0" \
	    "Shift bitmap left (Ctrl+H)" -activebackground white
    ::poToolbar::AddButton $btnFrame.right [::poBmpData::right] "ShiftBmp 1 0" \
	    "Shift bitmap right (Ctrl+L)" -activebackground white
    ::poToolbar::AddButton $btnFrame.up [::poBmpData::up]      "ShiftBmp 0 -1" \
	    "Shift bitmap up (Ctrl+K)" -activebackground white
    ::poToolbar::AddButton $btnFrame.down [::poBmpData::down]  "ShiftBmp 0 1" \
	    "Shift bitmap down (Ctrl+J)" -activebackground white

    # Add new toolbar group and associated buttons.
    set btnFrame [::poToolbar::AddGroup .po.toolfr]

    ::poToolbar::AddButton $btnFrame.info [::poBmpData::info] BmpInfo \
	    "Info about bitmap (Ctrl+I)" -activebackground white
    label .po.workfr.infofr.label -text Status -anchor w
    pack  .po.workfr.infofr.label -fill x -in .po.workfr.infofr

    set normal [image create photo -width $gBmp(xsize) -height $gBmp(ysize)]
    set invert [image create photo -width $gBmp(xsize) -height $gBmp(ysize)]
    $normal blank
    $invert blank
    label .po.workfr.prefr.normal -image $normal -borderwidth 0 -bg white
    label .po.workfr.prefr.invert -image $invert -borderwidth 0 -bg black
    pack .po.workfr.prefr.normal .po.workfr.prefr.invert -padx 4 -pady 2
    set gBmp(imgNormal) $normal
    set gBmp(imgInvert) $invert
...
proc AddRecentFiles { menuId delOldEntries } {
	...
	AddMenuCmd $menuId $curFile "" [list OpenBmp $curFile]
    }
}
...
proc AddRecentDirs { menuId delOldEntries } {
	...
	AddMenuCmd $menuId $curDir "" [list OpenBrowseWin $curDir]
    }
}
...
proc GetColor { buttonId colorType } {
    global gPo

    set newColor [tk_chooseColor -initialcolor $gPo($colorType)]
    if { [string compare $newColor ""] != 0 } {
	set gPo($colorType) $newColor
...
proc GetPluginDir {} {
    ...
    set tmpDir [::poTree::GetDir -initialdir $gPo(pluginDir) -showfiles 1 \
                                 -title "Select plugin directory"]
    if { [string compare $tmpDir ""] != 0 && \
	 [file isdirectory $tmpDir] } {
	set gPo(pluginDir) $tmpDir
	FillPluginMenu
    }
}
...
proc ShowMiscSettWin {} {
    ...
    # Part 2: Max Undo settings
    incr row
    frame $tw.fr$row
    grid $tw.fr$row -row $row -column 1 -sticky news

    entry $tw.fr$row.e -textvariable gPo(maxUndo)
    pack $tw.fr$row.e -side top -fill both -expand 1 \
	 -padx 3 -pady 3 -in $tw.fr$row

    set tmpList [list [list gPo(maxUndo)] [list $gPo(maxUndo)]]
    lappend varList $tmpList

    # Part 3: Number of entries in Reopen menu
    incr row
    frame $tw.fr$row
    grid $tw.fr$row -row $row -column 1 -sticky news
    ...
    # Part 4: Number of entries in Rebrowse menu
    incr row
    frame $tw.fr$row
    grid $tw.fr$row -row $row -column 1 -sticky news
    ...
    # Part 5: Plugin directory settings
    incr row
    frame $tw.fr$row -relief ridge -borderwidth 1
    grid $tw.fr$row -row $row -column 1 -sticky news

    entry  $tw.fr$row.e -textvariable gPo(pluginDir)
    button $tw.fr$row.b -text "..." -command GetPluginDir -borderwidth 0
    pack $tw.fr$row.e -side left -anchor w -in $tw.fr$row -expand 1 -fill x
    pack $tw.fr$row.b -side left -anchor w -in $tw.fr$row
    ::poToolhelp::AddBinding $tw.fr$row.e [list $gPo(pluginDir)]
    ::poToolhelp::AddBinding $tw.fr$row.b "Click to select new plugin directory"

    set tmpList [list [list gPo(pluginDir)] [list $gPo(pluginDir)]]
    lappend varList $tmpList
...
proc ShowColorSettWin {} {
    global gPo

    set tw .poBitmap:colorSettWin
    ...
    toplevel $tw
    wm title $tw "Color settings"
    wm resizable $tw false false
    wm geometry $tw [format "+%d+%d" $gPo(colorSett,x) $gPo(colorSett,y)]
    ...
    button $tw.fr$row.b1 -text "..." -bg $gPo(gridColor) \
                               -command "GetColor $tw.fr$row.b1 gridColor"
    ::poToolhelp::AddBinding $tw.fr$row.b1 "Click to select new color"
    pack $tw.fr$row.b1 \
	 -side top -fill both -expand 1 -padx 3 -pady 3 -in $tw.fr$row 
    ...
    button $tw.fr$row.b1 -text "..." -bg $gPo(onColor) \
                               -command "GetColor $tw.fr$row.b1 onColor"
    ::poToolhelp::AddBinding $tw.fr$row.b1 "Click to select new color"
    pack $tw.fr$row.b1 \
	 -side top -fill both -expand 1 -padx 3 -pady 3 -in $tw.fr$row 
    ...
    frame $tw.fr$row
    grid $tw.fr$row -row $row -column 1 -sticky news
    button $tw.fr$row.b1 -text "..." -bg $gPo(offColor) \
                               -command "GetColor $tw.fr$row.b1 offColor"
    ::poToolhelp::AddBinding $tw.fr$row.b1 "Click to select new color"
    pack $tw.fr$row.b1 \
	 -side top -fill both -expand 1 -padx 3 -pady 3 -in $tw.fr$row 
    ...
    frame $tw.fr$row
    grid $tw.fr$row -row $row -column 1 -sticky news
    button $tw.fr$row.b1 -text "..." -bg $gPo(cursorColor) \
                               -command "GetColor $tw.fr$row.b1 cursorColor"
    ::poToolhelp::AddBinding $tw.fr$row.b1 "Click to select new color"
    pack $tw.fr$row.b1 \
	 -side top -fill both -expand 1 -padx 3 -pady 3 -in $tw.fr$row 

    set tmpList [list [list gPo(gridColor)] [list $gPo(gridColor)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(onColor)] [list $gPo(onColor)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(offColor)] [list $gPo(offColor)]]
    lappend varList $tmpList
    set tmpList [list [list gPo(cursorColor)] [list $gPo(cursorColor)]]
    lappend varList $tmpList
...
proc CreateGrid { canvasId xsize ysize pixsize } {
    global gPo gBmp
	...
	$canvasId create rectangle $x1 $y1 $x2 $y2 \
		  -outline $gPo(gridColor) -fill $gPo(offColor) \
                  -tags [list [format "x_%d_y_%d" $x $y] "rect"]
	}
    }
    $canvasId configure -width [expr $xsize * $pixsize] -height [expr $ysize * $pixsize]
    $canvasId configure -scrollregion "0 0 [expr $xsize * $pixsize] [expr $ysize * $pixsize]"
...
proc ShiftBmp { sx sy } {
    ...
    set xsize $gBmp(xsize)
    set ysize $gBmp(ysize)
    # Shift horizontally: sx > 0 is right, sx < 0 is left
    if { $sx > 0 } {
	set col 0
    } elseif { $sx < 0 } {
	set col [expr $xsize -1]
    }
    if { $sx != 0 } {
 	if { $sx > 0 } {
	    set xsize2 [expr {$xsize -2}]
	    set xsize1 [expr {$xsize -1}]
	    for { set y 0 } { $y < $ysize } { incr y } {
		set x1 $xsize2
		for { set x $xsize1 } { $x > 0 } { incr x -1 } {
		    set gBmp($x,$y) $gBmp($x1,$y)
		    incr x1 -1
		}
	    }
	} else {
	...
	set row [expr $ysize -1]
    }

    if { $sy != 0 } {
 	if { $sy > 0 } {
	    set ysize1 [expr {$ysize -1}]
	    set ysize2 [expr {$ysize -2}]
	    for { set x 0 } { $x < $xsize } { incr x } {
		set y1 $ysize2
		for { set y $ysize1 } { $y > 0 } { incr y -1 } {
		    set gBmp($x,$y) $gBmp($x,$y1)
		    incr y1 -1
		}
	    }
	} else {
    ...
    set canvasId $gBmp(canvId)
    set onColor  $gPo(onColor)
    set offColor $gPo(offColor)

    for { set y 0 } { $y < $ysize } { incr y } {
	for { set x 0 } { $x < $xsize } { incr x } {
	    set tag [format "x_%d_y_%d" $x $y]
	    if { $gBmp($x,$y) } {
		$canvasId itemconfigure $tag -fill $onColor
	    } else {
		$canvasId itemconfigure $tag -fill $offColor
	    }
	}
    }
    UpdatePreview
...
proc SetPixel { w x y onOff } {
    global gPo gBmp gCursor
	...
	if { $onOff } {
	    set gBmp($cx,$cy) 255
	    $w itemconfigure $item -fill $gPo(onColor)
	} else {
	    set gBmp($cx,$cy) 0
	    $w itemconfigure $item -fill $gPo(offColor)
	}
	poplGoto $cx $cy
        MoveCursor 0 0
    }
...
proc Undo {} {
    global gUndo gUndoNo

    if { $gUndoNo > 0 } {
	incr gUndoNo -1
	CreateBmpFromImg $gUndo($gUndoNo)
    } else {
...
proc Redo {} {
    global gPo gUndo gUndoNo

    if { $gUndoNo < $gPo(maxUndo) && [info exists gUndo([expr $gUndoNo +1])] } {
	incr gUndoNo
	CreateBmpFromImg $gUndo($gUndoNo)
    } else {
...
proc ShowUndoImgs {} {
    ...
    toplevel .sui
    for { set i 0 } { $i <= $gUndoNo } { incr i } {
	label .sui.n$i -text "$i: $gUndo($i)"
	label .sui.l$i -image $gUndo($i) -bg black
...
proc CutImg { xtl ytl xbr ybr } {
	...
	for { set x $xtl } { $x <= $xbr } { incr x } {
	    set gBmp($x,$y) 0
	    set tag [format "x_%d_y_%d" $x $y]
	    $gBmp(canvId) itemconfigure $tag -fill $gPo(offColor)
	}
    }
    UpdatePreview
    SaveUndoImg
...
proc CopyImg { xtl ytl xbr ybr } {
    global gImgBuffer gBmp gPo
    
    if { [info exists gImgBuffer] } {
	image delete $gImgBuffer
    }

    set gImgBuffer [image create photo] 
    $gImgBuffer blank
    ::poLog::Info "Copy image area ($xtl, $ytl) ($xbr, $ybr) into buffer."
    $gImgBuffer copy $gBmp(imgInvert) \
                -from $xtl $ytl [expr $xbr +1] [expr $ybr +1]
}

proc AskCutImg {} {
    set topLeft [poplMouse "Cut: Select top-left corner of area"]
    if { [llength $topLeft] != 0 } {
	set bottomRight [poplMouse "Cut: Select bottom-right corner of area"]
	if { [llength $bottomRight] != 0 } {
	    ...
	    WriteInfoStr "Cutting not possible: No bottom-right corner specified."
	}
    } else {
	WriteInfoStr "Cuting not possible: No top-left corner specified."
    }
}

proc AskCopyImg {} {
    set topLeft [poplMouse "Copy: Select top-left corner of area"]
    if { [llength $topLeft] != 0 } {
	set bottomRight [poplMouse "Copy: Select bottom-right corner of area"]
	if { [llength $bottomRight] != 0 } {
	    set x1 [lindex $topLeft 0]
	    set y1 [lindex $topLeft 1]
	    set x2 [lindex $bottomRight 0]
	    ...
	    WriteInfoStr "Copying not possible: No bottom-right corner specified."
	}
    } else {
	WriteInfoStr "Copying not possible: No top-left corner specified."
    }
}
...
proc PasteImg { xtl ytl } {
    ...
    set xsize [image width  $gImgBuffer] 
    set ysize [image height $gImgBuffer] 
	...
	for { set xd $xtl ; set xs 0 } \
	    ...
	    set tag [format "x_%d_y_%d" $xd $yd]
	    set pix [$gImgBuffer get $xs $ys]
	    set r [lindex $pix 0]
	    set g [lindex $pix 1]
	    set b [lindex $pix 2]
	    if { $r == 255 && $g == 255 && $b == 255 } {
		set gBmp($xd,$yd) 255
		$gBmp(canvId) itemconfigure $tag -fill $gPo(onColor)
	    } else {
		set gBmp($xd,$yd) 0
		$gBmp(canvId) itemconfigure $tag -fill $gPo(offColor)
	    }
...
proc AskPasteImg {} {
    set topLeft [poplMouse "Paste: Select top-left corner of area"]
    if { [llength $topLeft] != 0 } {
	PasteImg [lindex $topLeft 0] [lindex $topLeft 1]
    } else {
	WriteInfoStr "Pasting not possible: No top-left corner specified."
    }
}

proc SaveUndoImg {} {
    global gUndo gUndoNo gBmp gPo
    
    incr gUndoNo
    if { $gUndoNo >= $gPo(maxUndo) } {
	set gUndoNo [expr $gPo(maxUndo) -1]
	image delete $gUndo(0)
	for { set n 0 } { $n < $gUndoNo } { incr n } {
	    set n1 [expr $n +1]
	    set gUndo($n) $gUndo($n1)
	}
    }
    UpdateMainTitle 1

    set gUndo($gUndoNo) [image create photo] 
    ::poLog::Info "Saving image for undo no $gUndoNo ($gUndo($gUndoNo))"
    $gUndo($gUndoNo) copy $gBmp(imgInvert)
    # ShowUndoImgs
}
...
proc UpdatePreview {} {
    ...
    $gBmp(imgNormal) blank
    $gBmp(imgInvert) blank
    set xsize $gBmp(xsize)
    set ysize $gBmp(ysize)
    for { set y 0 } { $y < $ysize } { incr y } {
	for { set x 0 } { $x < $xsize } { incr x } {
	    if { $gBmp($x,$y) } {
		$gBmp(imgNormal) put "#000000" -to $x $y
		$gBmp(imgInvert) put "#FFFFFF" -to $x $y
	    }
	}
    }
}
...
proc PreviewNewSize {} {
    ...
    ::poLog::Info "PreviewNewSize"
    $gBmp(imgNormal) configure -width $gBmp(xsize) -height $gBmp(ysize)
    $gBmp(imgInvert) configure -width $gBmp(xsize) -height $gBmp(ysize)
    $gBmp(imgNormal) blank
    $gBmp(imgInvert) blank
    UpdatePreview
    SaveUndoImg
...
proc InvertBmp {} {
    ...
    for { set y 0 } { $y < $gBmp(ysize) } { incr y } {
	for { set x 0 } { $x < $gBmp(xsize) } { incr x } {
	    set tag [format "x_%d_y_%d" $x $y]
	    if { $gBmp($x,$y) == 0 } {
		set gBmp($x,$y) 255
		$gBmp(canvId) itemconfigure $tag -fill $gPo(onColor)
...
proc FlipHori {} {
    ...
    set yhalf [expr $gBmp(ysize) / 2]
    for { set x 0 } { $x < $gBmp(xsize) } { incr x } {
	set y2 [expr $gBmp(ysize) -1]
	for { set y1 0 } { $y1 < $yhalf } { incr y1 } {
	    set tag1 [format "x_%d_y_%d" $x $y1]
	    set tag2 [format "x_%d_y_%d" $x $y2]
	    set tmp $gBmp($x,$y1)
	    set gBmp($x,$y1) $gBmp($x,$y2)
	    set gBmp($x,$y2) $tmp
	    if { $gBmp($x,$y1) == 0 } {
		$gBmp(canvId) itemconfigure $tag1 -fill $gPo(offColor)
	    } else {
		$gBmp(canvId) itemconfigure $tag1 -fill $gPo(onColor)
	    }
	    if { $gBmp($x,$y2) == 0 } {
		$gBmp(canvId) itemconfigure $tag2 -fill $gPo(offColor)
	    } else {
		$gBmp(canvId) itemconfigure $tag2 -fill $gPo(onColor)
	    }
	    incr y2 -1
	}
    }
...
proc FlipVert {} {
    ...
    set xhalf [expr $gBmp(xsize) / 2]
    for { set y 0 } { $y < $gBmp(ysize) } { incr y } {
	set x2 [expr $gBmp(xsize) -1]
	for { set x1 0 } { $x1 < $xhalf } { incr x1 } {
	    set tag1 [format "x_%d_y_%d" $x1 $y]
	    set tag2 [format "x_%d_y_%d" $x2 $y]
	    set tmp $gBmp($x1,$y)
	    set gBmp($x1,$y) $gBmp($x2,$y)
	    set gBmp($x2,$y) $tmp
	    if { $gBmp($x1,$y) == 0 } {
		$gBmp(canvId) itemconfigure $tag1 -fill $gPo(offColor)
	    } else {
...
proc ClearBmp {} {
    ...
    CreateGrid $gBmp(canvId) $gBmp(xsize) $gBmp(ysize) $gBmp(pixsize)
    set gBmp(defName) "untitled.xbm"
    UpdatePreview
    MoveCursor 0 0
    SaveUndoImg
}
...
proc CreateNewBitmap {} {
    global gPo gBmp gUndoNo

    set gBmp(xsize)   $gPo(new,w)
    set gBmp(ysize)   $gPo(new,h)
    set gBmp(pixsize) $gPo(new,s)
    CreateGrid $gBmp(canvId) $gBmp(xsize) $gBmp(ysize) $gBmp(pixsize)
    set gBmp(defName) "untitled.xbm"
    DestroyNewBitmapWin
    PreviewNewSize
...
proc ShowNewBitmapWin { cmd title { noChars 20 } } {
    ...
    bind  $tw.e$row <KeyPress-Escape> DestroyNewBitmapWin
    ...
    incr row
    entry $tw.e$row -textvariable gPo(new,s)
    $tw.e$row configure -width $noChars
    bind  $tw.e$row <KeyPress-Return> "$cmd"
    bind  $tw.e$row <KeyPress-Escape> DestroyNewBitmapWin
    grid  $tw.e$row -row $row -column 1 -sticky nwe
    ...
    frame $tw.fr$row
    grid  $tw.fr$row -row $row -column 0 -columnspan 2 -sticky news
    button $tw.fr$row.b1 -text "Cancel" -command DestroyNewBitmapWin
...
proc DestroyNewBitmapWin {} {
    destroy .po:newBitmapWin
}

proc NewBmp {} {
    ShowNewBitmapWin CreateNewBitmap "New bitmap"
}
...
proc ReadBmp { bmpFile } {
    ...
    if { $retVal != 0 } {
        error "Can't read bitmap file $bmpFile"
    }
    gets $fp line
...
proc SaveBmpPackage { fileList outFile packageName namespaceName versionNum } {
    ...
    foreach fileName $fileList {
	set bmpName  [file rootname [file tail $fileName]]
	set procName [format "%s::%s" $namespaceName $bmpName]
...
proc GetBmpPackageFile { dir } {
    ...
    set tmp [tk_getSaveFile -filetypes $fileTypes \
	      -initialfile $gPo(packageName).tcl -initialdir $gPo(lastDir) \
	      -title "Save bitmap package"]

    if { [string compare $tmp ""] != 0 } {
	SaveBmpPackage [GetSelBmps] $tmp $gPo(packageName) \
                       $gPo(namespaceName) $gPo(versionString)
    }
}

proc ShowBmpPackageWin {} {
    global gPo gBmp
    ...
    toplevel $tw
    wm title $tw "Bitmap package settings"
    wm resizable $tw false false
    # wm geometry $tw [format "+%d+%d" $sett(x) $sett(y)]
    ...
    frame $tw.fr$row
    grid  $tw.fr$row -row $row -column 1 -sticky nw
    entry $tw.fr$row.e -textvariable gPo(packageName) -bg white -width 20
    pack  $tw.fr$row.e -anchor w -in $tw.fr$row -padx $xpad -pady $ypad

    set tmpList [list [list gPo(packageName)] [list $gPo(packageName)]]
    lappend varList $tmpList
    ...
    frame $tw.fr$row
    grid  $tw.fr$row -row $row -column 1 -sticky nw
    entry $tw.fr$row.e -textvariable gPo(namespaceName) -bg white -width 20
    pack  $tw.fr$row.e -anchor w -in $tw.fr$row -padx $xpad -pady $ypad

    set tmpList [list [list gPo(namespaceName)] [list $gPo(namespaceName)]]
    lappend varList $tmpList
    ...
    frame $tw.fr$row
    grid  $tw.fr$row -row $row -column 1 -sticky nw
    entry $tw.fr$row.e -textvariable gPo(versionString) -bg white -width 20
    pack  $tw.fr$row.e -anchor w -in $tw.fr$row -padx $xpad -pady $ypad

    set tmpList [list [list gPo(versionString)] [list $gPo(versionString)]]
    lappend varList $tmpList
    ...
    button $tw.fr$row.b1 -text "Cancel" \
	   -command "CancelToplevel $tw $varList"
    button $tw.fr$row.b2 -text "OK" -command "OKToplevel $tw"
    bind  $tw <KeyPress-Escape> "CancelToplevel $tw $varList"
    pack $tw.fr$row.b1 $tw.fr$row.b2 -side left -fill x -padx 2 -expand 1
    wm protocol $tw WM_DELETE_WINDOW "CancelToplevel $tw $varList"
    focus $tw
}
...
proc GetSelBmps {} {
    ...
    set fileList {}
    foreach f [lsort [array names gBmpList]] {
	if { $gBmpList($f) } {
	    lappend fileList $f
	}
    }
...
proc SelUnselAllBmps { onOff } {
    ...
    foreach f [array names gBmpList] {
        set gBmpList($f) $onOff
    }
}
...
proc UpdateBmpDir { tw fr dir } {
    ...
    frame $fr -relief sunken
    pack  $fr -side top -fill both -expand 1 -pady 2
    set bmpfr [::poWin::CreateScrolledFrame $fr "Bitmap listing"]
    ...
    set bmpFileList [lsort [lindex \
			    [::poMisc::GetDirList $dir 0 1 1 1 * $extStr] 1] ]
	...
	if { $catchVal } {
		...
		-indicatoron false -image $bmp -variable gBmpList($f)
...
proc OpenBrowseWin { dir } {
    ...
    toplevel $tw
    wm title $tw "poBitmap browser: [file tail $dir]"
    wm resizable $tw true true
    wm geometry $tw [format "%dx%d+%d+%d" \
       $gPo(browse,w) $gPo(browse,h) $gPo(browse,x) $gPo(browse,y)]
    ...
    ::poToolbar::AddButton $btnFrame.open [::poBmpData::open] \
                           "OpenSelBmps" \
                           "Load selected bitmaps into editor ...(Ctrl+O)" \
		           -activebackground white
    bind $tw <Control-o> "OpenSelBmps"
    ::poToolbar::AddButton $btnFrame.save [::poBmpData::save] \
		           "GetBmpPackageFile [list $dir]" \
		           "Save selected bitmaps as package ...(Ctrl+S)" \
		           -activebackground white
    bind $tw <Control-s> "GetBmpPackageFile [list $dir]"

    set btnFrame [::poToolbar::AddGroup $tw.toolfr]
    ::poToolbar::AddButton $btnFrame.selall [::poBmpData::selectall] \
		           "SelAllBmps" \
		           "Select all bitmaps (Ctrl+A)" \
		           -activebackground white
    bind $tw <Control-a> "SelAllBmps"
    ::poToolbar::AddButton $btnFrame.unselall [::poBmpData::unselectall] \
		           "UnselAllBmps" \
		           "Unselect all bitmaps (Ctrl+U)" \
		           -activebackground white
    bind $tw <Control-u> "UnselAllBmps"
    ::poToolbar::AddButton $btnFrame.update [::poBmpData::update] \
		           "UpdateBmpDir $tw $tw.workfr [list $dir]" \
		           "Update bitmap listing (F5)" -activebackground white
    bind $tw <Key-F5> "UpdateBmpDir $tw $tw.workfr [list $dir]"

    set btnFrame [::poToolbar::AddGroup $tw.toolfr]
    ::poToolbar::AddButton $btnFrame.delete [::poBmpData::delete] \
		           "DelSelBmps $tw $tw.workfr [list $dir]" \
		           "Delete selected bitmap files (Del)" \
		           -activebackground white
    bind $tw <Delete> "DelSelBmps $tw $tw.workfr [list $dir]"

    UpdateBmpDir $tw $tw.workfr $dir

    bind $tw <Key-Escape> "DelBrowseWin $tw"
    wm protocol $tw WM_DELETE_WINDOW "DelBrowseWin $tw"
    focus $tw
}
...
proc CreateBmpFromImg { phImg } {
    ...
    set gBmp(xsize) [image width  $phImg] 
    set gBmp(ysize) [image height $phImg] 
    CreateGrid $gBmp(canvId) $gBmp(xsize) $gBmp(ysize) $gBmp(pixsize)
    for { set y 0 } { $y < $gBmp(ysize) } { incr y } {
	for { set x 0 } { $x < $gBmp(xsize) } { incr x } {
	    set tag [format "x_%d_y_%d" $x $y]
...
proc OpenBmp { imgName } {
    ...
    set retVal [catch {set phImg [ReadBmp $imgName]} err]
    if { $retVal != 0 } {
        if { $gPo(ext,Img,avail) } {
	    ::poLog::Info "Reading bitmap with Img extension"
	    set ext [file extension $imgName]
	    set fmtStr [::poImgtype::GetFmtByExt $ext]
	    if { [string compare $fmtStr ""] == 0 } {
	    ...
	    tk_messageBox -message "Can't read bitmap $imgName ($err)." \
				   -type ok -icon warning
	    focus .po
	    return
	}
    ...
    set gBmp(defName) [file tail $imgName]
    PreviewNewSize
    MoveCursor 0 0
    UpdateMainTitle
    image delete $phImg
...
proc OpenBmpFile { { useLastDir true } } {
    global gPo gBmp
 
    set imgName [GetFileName "Open bitmap" $useLastDir "open" $gBmp(defName)]
    if { $imgName != "" } {
        OpenBmp $imgName
    }
}

proc SaveBmpFile { { useLastDir true } } {
    global gPo gBmp
 
    set imgName [GetFileName "Save bitmap" $useLastDir "save" $gBmp(defName)]
    if { $imgName != "" } {
	if { $gPo(ext,Img,avail) } {
	    SaveBmp .po $gBmp(imgNormal) $imgName
	} else {
	    WriteBmp $gBmp(imgInvert) $imgName
	}
	set gBmp(defName) $imgName
	UpdateMainTitle
    }
    return $imgName
}
...
proc BmpInfo {} {
    ...
    label $tw.fr0.ll$row -text "Filename:"
    label $tw.fr0.lr$row -text $gBmp(defName)
    grid  $tw.fr0.ll$row -row $row -column 0 -sticky nw
    grid  $tw.fr0.lr$row -row $row -column 1 -sticky nw
    incr row

    label $tw.fr0.ll$row -text "Width:"
    label $tw.fr0.lr$row -text $gBmp(xsize)
    grid  $tw.fr0.ll$row -row $row -column 0 -sticky nw
    grid  $tw.fr0.lr$row -row $row -column 1 -sticky nw
    incr row

    label $tw.fr0.ll$row -text "Height:"
    label $tw.fr0.lr$row -text $gBmp(ysize)
    grid  $tw.fr0.ll$row -row $row -column 0 -sticky nw
    grid  $tw.fr0.lr$row -row $row -column 1 -sticky nw
    incr row
...
proc LoadSettings {} {
    global gPo gBmp tcl_platform gUndoNo

    # Init global variables not stored in the cfg file.
    set gPo(lastDir)   [list [pwd]]
    set gPo(pluginDir) [list [pwd]]
    ...
    set gPo(packageName)   "myBitmap"
    set gPo(namespaceName) "::myBitmap"
    set gPo(versionString) "1.0"

    set gPo(new,w) $gBmp(xsize)
    set gPo(new,h) $gBmp(ysize)
    set gPo(new,s) $gBmp(pixsize)
    ...
    set gPo(main,x)          10
    set gPo(main,y)          30
    set gPo(browse,w)        300
    set gPo(browse,h)        300
    set gPo(browse,x)        20
    set gPo(browse,y)        40
    set gPo(miscSett,x)      90
    set gPo(miscSett,y)      40
    set gPo(colorSett,x)     100
    set gPo(colorSett,y)     50
    set gPo(onColor)         "black"
    set gPo(offColor)        "white"
    set gPo(gridColor)       "grey"
    set gPo(cursorColor)     "red"
...
proc SaveSettings {} {
	...
	if { [winfo exists .poBitmap:browseDirWin] } {
	    puts $fp "set gPo(browse,x)  \{[winfo x .poBitmap:browseDirWin]\}"
	    puts $fp "set gPo(browse,y)  \{[winfo y .poBitmap:browseDirWin]\}"
	    puts $fp "set gPo(browse,w)  \{[winfo width .poBitmap:browseDirWin]\}"
	    puts $fp "set gPo(browse,h)  \{[winfo height .poBitmap:browseDirWin]\}"
	} else {
	    puts $fp "set gPo(browse,x)  \{$gPo(browse,x)\}"
	    puts $fp "set gPo(browse,y)  \{$gPo(browse,y)\}"
	    puts $fp "set gPo(browse,w)  \{$gPo(browse,w)\}"
	    puts $fp "set gPo(browse,h)  \{$gPo(browse,h)\}"
	}
	if { [winfo exists .poBitmap:miscSettWin] } {
	    puts $fp "set gPo(miscSett,x)  \{[winfo x .poBitmap:miscSettWin]\}"
	    puts $fp "set gPo(miscSett,y)  \{[winfo y .poBitmap:miscSettWin]\}"
	} else {
	    puts $fp "set gPo(miscSett,x)  \{$gPo(miscSett,x)\}"
	    puts $fp "set gPo(miscSett,y)  \{$gPo(miscSett,y)\}"
	}

	if { [winfo exists .poBitmap:colorSettWin] } {
	    puts $fp "set gPo(colorSett,x) \{[winfo x .poBitmap:colorSettWin]\}"
	    puts $fp "set gPo(colorSett,y) \{[winfo y .poBitmap:colorSettWin]\}"
	} else {
	    puts $fp "set gPo(colorSett,x) \{$gPo(colorSett,x)\}"
	    puts $fp "set gPo(colorSett,y) \{$gPo(colorSett,y)\}"
	}

	puts $fp "set gPo(cursorColor) \{$gPo(cursorColor)\}"
	puts $fp "set gPo(gridColor)   \{$gPo(gridColor)\}"
	puts $fp "set gPo(onColor)     \{$gPo(onColor)\}"
	puts $fp "set gPo(offColor)    \{$gPo(offColor)\}"
	puts $fp "set gPo(showSplash)  \{$gPo(showSplash)\}"
	puts $fp "set gPo(maxUndo)     \{$gPo(maxUndo)\}"

	puts $fp "set gPo(lastDir)       \{$gPo(lastDir)\}"
	puts $fp "set gPo(pluginDir)     \{$gPo(pluginDir)\}"
	...
	puts $fp "set gPo(packageName)   \{$gPo(packageName)\}"
	puts $fp "set gPo(namespaceName) \{$gPo(namespaceName)\}"
	puts $fp "set gPo(versionString) \{$gPo(versionString)\}"
...
Just a list of shortcuts not available via menu:\n
...
Shift + Arrow key: Clear pixel under cursor, then move cursor."
    ...
    ::poWin::CreateHelpWin $helpStr
}
 
proc HelpProg { {splashWin ""} } {
    global gPo

    ::poSoftLogo::ShowLogo "$gPo(appName) Version 0.2" \
        "Copyright 2001-2008 Paul Obermeier" \
        $splashWin
    if { [string compare $splashWin ""] != 0 } {
        ::poSoftLogo::DestroyLogo
    }
...
proc MoveCursor { dx dy } {
    global gCursor gBmp gPo

    set tag [format "x_%d_y_%d" $gCursor(x) $gCursor(y)]
    $gBmp(canvId) itemconfigure $tag -outline $gPo(gridColor)
    $gBmp(canvId) itemconfigure $tag -width 1
    ...
    if { $gCursor(x) >= $gBmp(xsize) } {
        set gCursor(x) [expr $gBmp(xsize) -1]
    }
    if { $gCursor(y) >= $gBmp(ysize) } {
        set gCursor(y) [expr $gBmp(ysize) -1]
    }
    set tag [format "x_%d_y_%d" $gCursor(x) $gCursor(y)]
    $gBmp(canvId) itemconfigure $tag -outline $gPo(cursorColor)
    $gBmp(canvId) itemconfigure $tag -width 2

    WriteInfoStr "Cursor position: $gCursor(x) $gCursor(y)"
}

proc DrawPixel { onOff { updatePreview 1 } } {
    global gCursor gPo gBmp

    set tag [format "x_%d_y_%d" $gCursor(x) $gCursor(y)]
    if { $onOff } {
	set gBmp($gCursor(x),$gCursor(y)) 255
	$gBmp(canvId) itemconfigure $tag -fill $gPo(onColor)
    } else {
	set gBmp($gCursor(x),$gCursor(y)) 0
	$gBmp(canvId) itemconfigure $tag -fill $gPo(offColor)
    }
    if { $updatePreview } {
	UpdatePreview
...
#       Name:           poplUpdate
#
#       Usage:          Update canvas and preview windows.
#
#       Synopsis:       proc poplUpdate {}
#
#       Description:	Update canvas and preview windows.
#
#       Return Value:   None.
#
#       See also: 	poplSaveUndo
#
###########################################################################
proc poplUpdate {} {
    UpdatePreview
...
#       Name:           poplSaveUndo
#
#       Usage:          Save current drawing status for undo operation.
#
#       Synopsis:       proc poplSaveUndo {}
#
#       Description:	Save current drawing status for undo operation.
#
#       Return Value:   None.
#
#       See also: 	poplUpdate
...
#       Name:           poplMouse
#
#       Usage:          Get cursor position by mouse.
#
#       Synopsis:       proc poplMouse { msg }
#
...
#
#       Return Value:   The pixel the user has selected as a list of row and
...
###########################################################################
proc poplMouse { msg } {
    global gBmp gCursor gPo

    set w $gBmp(canvId)
    
    set gPo(drawMode) 0
    $w configure -cursor crosshair
    bind $w <ButtonRelease-1> "SetMousePos %W %x %y"
    bind $w <KeyPress-Escape> "CancelMousePos"
    ...
    WriteInfoStr ""
    if { $gCursor(mouse) } {
        return [list $gCursor(x) $gCursor(y)]
    } else {
        return {}
    }
}
...
#       Name:           poplWidth
#
#       Usage:          Get the width of the current bitmap.
#
#       Synopsis:       proc poplWidth {}
#
#       Description:	Get the width of the current bitmap.
#
...
#       Name:           poplHeight
#
#       Usage:          Get the height of the current bitmap.
#
#       Synopsis:       proc poplHeight {}
#
#       Description:	Get the height of the current bitmap.
#
#       Return Value:   integer.
#
#       See also: 	poplWidth
...
#       Name:           poplGetPix
#
#       Usage:          Get the pixel value at the current cursor position.
#
#       Synopsis:       proc poplGetPix {}
#
#       Description:	Get the pixel value at the current cursor position.
#
#       Return Value:   integer.
#
#       See also: 	poplMouse
...
proc poplGetPix {} {
    ...
    return $gBmp($gCursor(x),$gCursor(y))
}
...
#       Name:           poplDraw
#
#       Usage:          Set pixel at current cursor position to opaque.
#
#       Synopsis:       proc poplDraw { { withUpdate 0 } }
#
#       Description:	withUpdate: boolean, optional (0)
#
#			If withUpdate is set to 1, poplUpdate is called after
#			drawing.
#
#       Return Value:   None.
#
...
#
###########################################################################
proc poplDraw { { withUpdate 0 } } {
    DrawPixel 1 $withUpdate
}
...
#       Name:           poplClear
#
#       Usage:          Set pixel at current cursor position to transparent.
#
#       Synopsis:       proc poplClear { { withUpdate 0 } }
#
#       Description:	withUpdate: boolean, optional (0)
#
#			If withUpdate is set to 1, poplUpdate is called after
...
#
###########################################################################
proc poplClear { { withUpdate 0 } } {
    DrawPixel 0 $withUpdate
}
...
#       Name:           poplClearBmp
#
#       Usage:          Clear the whole bitmap.
#
#       Synopsis:       proc poplClearBmp {}
#
#       Description:	Clear the whole bitmap.
#
#       Return Value:   None.
#
#       See also: 	poplUpdate
...
#       Name:           poplInvertBmp
#
#       Usage:          Invert the whole bitmap.
#
#       Synopsis:       proc poplInvertBmp {}
#
#       Description:	Invert the whole bitmap.
#
#       Return Value:   None.
#
#       See also: 	poplUpdate
...
#       Name:           poplShiftBmp
#
#       Usage:          Shift the whole bitmap.
#
#       Synopsis:       proc poplShiftBmp { { x 1 } { y 0 } }
#
#       Description:	x: integer, optional (1)
#			y: integer, optional (0)
#
#			If called without parameters, the bitmap is shifted one
#			pixel to the right.
#
#       Return Value:   None.
#
...
#       Name:           poplGoto
#
#       Usage:          Set the cursor position.
#
#       Synopsis:       proc poplGoto { row { col -1 } }
#
#       Description:	row: integer
#			col: integer, optional (-1)
#
#			Set cursor position to (row,col).
#			If only one parameter is given, it is interpreted as
#			a list of (row,column) values, as returned by poplMouse.
#
#       Return Value:   None.
#
#       See also: 	poplMouse
...
#       Name:           poplUp
#
#       Usage:          Move the cursor up.
#
#       Synopsis:       proc poplUp { { noRows 1 } }
#
#       Description:	noRows: integer, optional (1)
#
...
#       Name:           poplDown
#
#       Usage:          Move the cursor down.
#
#       Synopsis:       proc poplDown { { noRows 1 } }
#
#       Description:	noRows: integer, optional (1)
#
...
#       Name:           poplLeft
#
#       Usage:          Move the cursor left.
#
#       Synopsis:       proc poplLeft { { noCols 1 } }
#
#       Description:	noCols: integer, optional (1)
#
...
#       Name:           poplRight
#
#       Usage:          Move the cursor right.
#
#       Synopsis:       proc poplRight { { noCols 1 } }
#
#       Description:	noCols: integer, optional (1)
#
...
set gPo(appName)   "poBitmap"	; # Name of main application
...
if { $argc >= 1 } {
	...
	if { [string compare -length 1 $curParam "-"] == 0 } {
	    ...
	    if { [file isdirectory $curParam] } {
	        set dirToOpen $curParam
	    } elseif { [file readable $curParam] } {
	        set fileToOpen $curParam
	    }
	}
	incr curArg
    }
...
if { [string compare $fileToOpen ""] != 0 } {
    OpenBmp $fileToOpen
} else {
    PreviewNewSize
}
if { [string compare $dirToOpen ""] != 0 } {
    set curDir [::poMisc::FileSlashName $dirToOpen]
    OpenBrowseWin $curDir
    if { [lsearch -exact $gPo(recentDirList) $curDir] < 0 } {
	set gPo(recentDirList) [linsert $gPo(recentDirList) 0 $curDir]
	set gPo(recentDirList) [lrange $gPo(recentDirList) 0 \
