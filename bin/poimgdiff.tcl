...
#
#	Module:		poImgdiff
#	Copyright:	Paul Obermeier 2000-2008
#	Filename:	poImgdiff.tcl
#
...
#			Images can be loaded with the GUI or by specifying 
#			them on the command line.
#			Besides comparing the images visually, it offers the
#			possibility to generate a difference image and the 
#			histograms of the images.
#			These operations are available with standard Tcl/Tk, but
#			are very slow for images greater than 200 square pixels.
#			If using the poImg extension, they are quite fast.
#
#			Image formats available:
#			    Tcl/Tk only:
#				GIF, XPM
#			    If you have the Img extension:
#				JPEG, TIFF, BMP, ... 
#			    If you have the poImg extension:
#				Targa, SGI
#
#	Additional documentation:
#			No further documentation, should be self-explaining.
#
#	Exported functions:
...
InitPackages Tk poTcllib poTklib poExtlib img::raw img::dted Img poImg poImglib
...
proc NewMarker { w name type color {x1 0} {y1 0} {x2 0} {y2 0} } {
    switch $type {
	"rectangle" {
	    $w create rectangle $x1 $y1 $x2 $y2 -outline $color -tags $name
	}
    }
...
proc PrintPixelValue { canvasId x y } {
    ...
    DrawMarker $gPo(left)  LeftRect  $px $py $gPo(zoom) $gPo(zoomRectSize)
    DrawMarker $gPo(right) RightRect $px $py $gPo(zoom) $gPo(zoomRectSize)

    set rgbStr "Position ($px,$py): "
    if { [info exists gPo($gPo(left),photo)] } {
	set retVal [catch {set rgb [$gPo($gPo(left),photo) get $px $py] }]
	if { $retVal == 0 } {
	    append rgbStr "([lindex $rgb 0], [lindex $rgb 1], [lindex $rgb 2])"
	}
    }
    if { [info exists gPo($gPo(right),photo)] } {
	set retVal [catch {set rgb [$gPo($gPo(right),photo) get $px $py] }]
	if { $retVal == 0 } {
	    append rgbStr "([lindex $rgb 0], [lindex $rgb 1], [lindex $rgb 2])"
	}
    }
...
proc PrintDiffPixelValue { canvasId photoId infoId x y } {
    global gPo

    set cx [expr int([$canvasId canvasx $x])]
    set cy [expr int([$canvasId canvasy $y])]
    set px [expr int([$canvasId canvasx $x] / $gPo($canvasId,zoomFactor))]
    set py [expr int([$canvasId canvasy $y] / $gPo($canvasId,zoomFactor))]
...
proc ShowMainWin {title} {
    ...
    grid .po.workfr.imgfr  -row 0 -column 0 -sticky nwse
    grid .po.workfr.infofr -row 1 -column 0 -sticky nwse 
    grid rowconfigure .po.workfr 0 -weight 1
    ...
    set gPo(lopenMenu) $lopenMenu
    set gPo(ropenMenu) $ropenMenu
    ...
    menu $lopenMenu -tearoff 0
    AddMenuCmd $lopenMenu "Browse ..."  "Ctrl+L" "OpenLeftImgFile"
    $lopenMenu add separator
    bind .po <Control-l>  "OpenLeftImgFile"

    menu $ropenMenu -tearoff 0
    AddMenuCmd $ropenMenu "Browse ..." "Ctrl+R" "OpenRightImgFile"
    $ropenMenu add separator
    bind .po <Control-r>  "OpenRightImgFile"

    menu $saveMenu -tearoff 0
    AddMenuCmd $saveMenu "left ..."  "Shift+L" "SaveLeftImg"
    AddMenuCmd $saveMenu "right ..." "Shift+R" "SaveRightImg"
    bind .po <L>  "SaveLeftImg"
    bind .po <R>  "SaveRightImg"

    set editMenu $hMenu.edit
    menu $editMenu -tearoff 0
    AddMenuCmd $editMenu "Clear left"   ""  ClearLeft
    AddMenuCmd $editMenu "Clear right"  ""  ClearRight

    set viewMenu  $hMenu.view
    set zoomMenu  $viewMenu.zoom
    set histoMenu $viewMenu.histo
    set diffMenu  $viewMenu.diff

    menu $viewMenu -tearoff 0
    $viewMenu add cascade -label "Zoom"       -menu $viewMenu.zoom
    $viewMenu add cascade -label "Histogram"  -menu $viewMenu.histo
    $viewMenu add cascade -label "Difference" -menu $viewMenu.diff

    AddMenuCmd $viewMenu "Image Info"   "Ctrl+I" ImgInfo
    ...
    AddMenuRadio $zoomMenu " 20%"  "" gPo(zoom) 0.20 "Zoom2 0.20"
    AddMenuRadio $zoomMenu " 25%"  "" gPo(zoom) 0.25 "Zoom2 0.25"
    AddMenuRadio $zoomMenu " 33%"  "" gPo(zoom) 0.33 "Zoom2 0.33"
    AddMenuRadio $zoomMenu " 50%"  "" gPo(zoom) 0.50 "Zoom2 0.50"
    AddMenuRadio $zoomMenu "100%"  "" gPo(zoom) 1.00 "Zoom2 1.00"
    AddMenuRadio $zoomMenu "200%"  "" gPo(zoom) 2.00 "Zoom2 2.00"
    AddMenuRadio $zoomMenu "300%"  "" gPo(zoom) 3.00 "Zoom2 3.00"
    AddMenuRadio $zoomMenu "400%"  "" gPo(zoom) 4.00 "Zoom2 4.00"
    AddMenuRadio $zoomMenu "500%"  "" gPo(zoom) 5.00 "Zoom2 5.00"

    menu $histoMenu -tearoff 0
    AddMenuCmd $histoMenu "Uniform scaling"     "Shift+H" "ShowHistogram2 0"
    AddMenuCmd $histoMenu "Logarithmic scaling" "Ctrl+H"  "ShowHistogram2 1"
    bind .po <H>         "ShowHistogram2 0"
    bind .po <Control-h> "ShowHistogram2 1"
    bind .po <Key-plus>   "ChangeZoom2 1"
    bind .po <Key-minus>  "ChangeZoom2 -1"

    menu $diffMenu -tearoff 0
    AddMenuCmd $diffMenu "No scaling"  "Ctrl+D"  "ShowDiffImg 0"
    AddMenuCmd $diffMenu "Color coded" "Shift+D" "ShowDiffImg 1"
    bind .po <Control-d> "ShowDiffImg 0"
    bind .po <G>         "ShowDiffImg 1"
    ...
    set canvasList [::poWin::CreateSyncCanvas .po.workfr.imgfr \
		    "Left new" "Right new" -borderwidth 2 -relief sunken]
    set gPo(left)  [lindex $canvasList 0]
    set gPo(right) [lindex $canvasList 1]
    set gPo($gPo(left),name)  "No image loaded"
    set gPo($gPo(right),name) "No image loaded"
    bind $gPo(left)  <Motion> "PrintPixelValue $gPo(left)  %x %y"
    bind $gPo(right) <Motion> "PrintPixelValue $gPo(right) %x %y"
    bind $gPo(left)  <Motion> "+UpdateZoomRect $gPo(left)  %x %y %X %Y"
    bind $gPo(right) <Motion> "+UpdateZoomRect $gPo(right) %x %y %X %Y"
    bind $gPo(left)  <1> "UpdateZoomRect $gPo(left)  %x %y %X %Y"
    bind $gPo(right) <1> "UpdateZoomRect $gPo(right) %x %y %X %Y"
    bind $gPo(left)  <Double-1> "OpenLeftImgFile"
    bind $gPo(right) <Double-1> "OpenRightImgFile"
    NewMarker $gPo(left) "LeftRect" "rectangle" $gPo(zoomRectColor) \
	      0 0 $gPo(zoomRectSize) $gPo(zoomRectSize)
    NewMarker $gPo(right) "RightRect" "rectangle" $gPo(zoomRectColor) \
	      0 0 $gPo(zoomRectSize) $gPo(zoomRectSize)

    set l $gPo(left)
    set r $gPo(right)
    catch {bind .po <Key-Down>      "::poWin::Yview $l $r scroll  1 units"}
    catch {bind .po <Key-Up>        "::poWin::Yview $l $r scroll -1 units"}
    catch {bind .po <Key-Left>      "::poWin::Xview $l $r scroll -1 units"}
    catch {bind .po <Key-Right>     "::poWin::Xview $l $r scroll  1 units"}
    catch {bind .po <Key-Page_Down> "::poWin::Yview $l $r scroll  1 pages"}
    catch {bind .po <Key-Page_Up>   "::poWin::Yview $l $r scroll -1 pages"}
    catch {bind .po <Key-Next>      "::poWin::Yview $l $r scroll  1 pages"}
    catch {bind .po <Key-Prior>     "::poWin::Yview $l $r scroll -1 pages"}
    catch {bind .po <Key-Home>      "::poWin::Yview $l $r moveto 0.0"}
    catch {bind .po <Key-End>       "::poWin::Yview $l $r moveto 1.0"}

    # Add new toolbar group and associated buttons.
    set btnFrame [::poToolbar::AddGroup .po.toolfr]

    ::poToolbar::AddButton $btnFrame.ol [::poBmpData::openleft] \
	    OpenLeftImgFile "Open left image file ... (Ctrl+L)" \
	    -activebackground white
    ::poToolbar::AddButton $btnFrame.or [::poBmpData::openright] \
	    OpenRightImgFile "Open right image file ... (Ctrl+R)" \
	    -activebackground white

    ::poToolbar::AddButton $btnFrame.sl [::poBmpData::saveleft] \
	    SaveLeftImg "Save left image to file ... (Shift+L)" \
	    -activebackground white
    ::poToolbar::AddButton $btnFrame.sr [::poBmpData::saveright] \
	    SaveRightImg "Save right image to file ... (Shift+R)" \
	    -activebackground white

    # Add new toolbar group and associated buttons.
    set btnFrame [::poToolbar::AddGroup .po.toolfr]

    ::poToolbar::AddButton $btnFrame.clearleft [::poBmpData::clearleft] \
            "ClearLeft" "Clear left image" -activebackground white
    ::poToolbar::AddButton $btnFrame.clearright [::poBmpData::clearright] \
            "ClearRight" "Clear right image" -activebackground white

    # Add new toolbar group and associated buttons.
    set btnFrame [::poToolbar::AddGroup .po.toolfr]

    ::poToolbar::AddButton $btnFrame.reset [::poBmpData::hundred] "Zoom2 1.00" \
	    "Reset image zoom" -activebackground white
    ::poToolbar::AddButton $btnFrame.histo [::poBmpData::histo] \
	    "ShowHistogram2 1" "Show logarithmic histogram (Ctrl+H)" \
	    -activebackground white
    ::poToolbar::AddButton $btnFrame.diff [::poBmpData::diff] "ShowDiffImg 0" \
	    "Show difference image (Ctrl+D)" -activebackground white
    ::poToolbar::AddButton $btnFrame.info [::poBmpData::info] ImgInfo \
	    "Info about images (Ctrl+I)" -activebackground white

    label .po.workfr.infofr.label -text Status -anchor w
    pack  .po.workfr.infofr.label -fill x -in .po.workfr.infofr

    AddRecentFiles $lopenMenu left false
    AddRecentFiles $ropenMenu right false

    WriteInfoStr $gPo(initStr)

    ::poWin::SetSyncTitle $gPo(left) \
			   $gPo($gPo(left),name) $gPo($gPo(right),name)
    $gPo(left)  config -cursor crosshair
    $gPo(right) config -cursor crosshair
}
...
proc ClearRecentImgList { leftMenuId rightMenuId } {
    global gPo 
...
proc AddRecentFiles { menuId side delOldEntries } {
    global gPo

    if { $delOldEntries } {
	DeleteMenuEntries $menuId 2
    }
    for { set i 0 } { $i < [llength $gPo(imgList)] } { incr i } {
        set curImg [lindex $gPo(imgList) $i]
	AddMenuCmd $menuId $curImg "" "ReadImg [list $curImg] $::gPo($side)"
    }
}

proc ConfigCanvas { photoId canvasId } {
    set iw [image width  $photoId]
    set ih [image height $photoId]
    set sw [winfo screenwidth  .po]
    set sh [winfo screenheight .po] 
    $canvasId configure -width  [::poMisc::Min $iw $sw] \
...
proc Zoom { phImg canvasId zoomValue } {
    ...
    $canvasId delete $canvasId

    set gPo($canvasId,zoomFactor) $zoomValue
    if { $zoomValue == 1.0 } {
        $canvasId create image 0 0 -anchor nw \
                      -image $phImg -tag $canvasId
        ConfigCanvas $phImg $canvasId
    } else {
	...
	if { [info exists gPo($canvasId,zoom)] } {
	    image delete $gPo($canvasId,zoom)
	}
	set newPhoto [image create photo -width $w -height $h]
        $newPhoto copy $phImg $cmd $sc $sc
        $canvasId create image 0 0 -anchor nw \
                      -image $newPhoto -tag $canvasId
        ConfigCanvas $newPhoto $canvasId
	set gPo($canvasId,zoom) $newPhoto
    }
}
...
proc Zoom2 { zoomValue } {
    global gPo

    if { ! [info exists gPo($gPo(left),photo)] || \
	 ! [info exists gPo($gPo(right),photo)] } {
	tk_messageBox -message "You must load 2 images first." \
		      -type ok -icon warning
	focus .po
	return
    }
    set leftId  $gPo($gPo(left),photo)
    set rightId $gPo($gPo(right),photo)
    $gPo(left)  delete $gPo(left)
    $gPo(right) delete $gPo(right)

    set gPo(zoom) $zoomValue
    if { $zoomValue == 1.0 } {
        $gPo(left) create image 0 0 -anchor nw \
                      -image $leftId -tag $gPo(left)
        $gPo(right) create image 0 0 -anchor nw \
                      -image $rightId -tag $gPo(right)
        ConfigCanvas $leftId  $gPo(left)
        ConfigCanvas $rightId $gPo(right)
    } else {
	set wl [expr int ([image width  $leftId] * $zoomValue)]
	set hl [expr int ([image height $leftId] * $zoomValue)]
	set wr [expr int ([image width  $rightId] * $zoomValue)]
	set hr [expr int ([image height $rightId] * $zoomValue)]
        if { $zoomValue < 1.0 } {
	    set sc [expr int (1.0 / $zoomValue)]
	    set cmd "-subsample"
	} elseif { $zoomValue > 1.0 } {
	...
	if { [info exists gPo($gPo(left),zoom)] } {
	    image delete $gPo($gPo(left),zoom)
	}
	if { [info exists gPo($gPo(right),zoom)] } {
	    image delete $gPo($gPo(right),zoom)
	}
	set newLeft  [image create photo -width $wl -height $hl]
	set newRight [image create photo -width $wr -height $hr]
        $newLeft  copy $leftId  $cmd $sc $sc
        $newRight copy $rightId $cmd $sc $sc
        $gPo(left) create image 0 0 -anchor nw \
                      -image $newLeft -tag $gPo(left)
        $gPo(right) create image 0 0 -anchor nw \
                      -image $newRight -tag $gPo(right)
        ConfigCanvas $newLeft  $gPo(left)
        ConfigCanvas $newRight $gPo(right)
	set gPo($gPo(left),zoom)  $newLeft
	set gPo($gPo(right),zoom) $newRight
    }
}
...
proc GetFileName { title useLastDir { mode "open" } { fileName "" } } {
    global gPo
	...
	if { $useLastDir } {
	    set imgName [tk_getOpenFile -filetypes $fileTypes \
			  -initialdir $gPo(lastDir) -title $title]
	} else {
	    set imgName [tk_getOpenFile -filetypes $fileTypes \
			  -initialdir [pwd] -title $title]
	}
    } else {
	if { $useLastDir } {
	    set imgName [tk_getSaveFile -filetypes $fileTypes \
			  -initialfile $fileName -initialdir $gPo(lastDir) \
			  -title $title]
	} else {
	    set imgName [tk_getSaveFile -filetypes $fileTypes \
			  -initialfile $fileName -initialdir [pwd] \
			  -title $title]
	}
    }
    if { $imgName != "" && $useLastDir } {
...
proc ShowImg { phImg str canvasId } {
    global gPo

    $canvasId delete $canvasId
    $canvasId create image 0 0 -anchor nw -image $phImg -tag $canvasId
    ConfigCanvas $phImg $canvasId
}

proc ReadImg { imgName canvasId } {
    global gPo
	...
	# Case (3)
		...
		$poImg img_photo $phImg $chanMap
	    }
	} else {
	    ::poLog::Warning "Can't read image $imgName ($err1)"
	    tk_messageBox -message "Can't read image \"$imgName\"" \
			  -type ok -icon warning
	    set found 0
	}
	...
	ShowImg $phImg $imgName $canvasId
	::poLog::Info "[expr [::poWatch::Lookup a] - $saveTime] sec: Displaying"
	if { $gPo(ext,poImg,avail) } {
	    if { [info exists gPo($canvasId,poImg)] } {
		deleteimg $gPo($canvasId,poImg)
		unset gPo($canvasId,poImg)
	    }
	    if { [info exists poImg] } {
		set gPo($canvasId,poImg) $poImg
	    }
	}
	if { [info exists gPo($canvasId,photo)] } {
	    image delete $gPo($canvasId,photo)
	}
        set gPo($canvasId,photo)  $phImg
        set gPo($canvasId,name)   $imgName
	set gPo($canvasId,width)  [image width  $gPo($canvasId,photo)]
	set gPo($canvasId,height) [image height $gPo($canvasId,photo)]
	::poWin::SetSyncTitle $gPo(left) \
			[file tail $gPo($gPo(left),name)] \
			[file tail $gPo($gPo(right),name)]
	if { [info exists gPo($gPo(left),photo)] && \
	     [info exists gPo($gPo(right),photo)] } {
	    Zoom2 $gPo(zoom)
	}
	...
	set curFile [::poMisc::FileSlashName $imgName]
        if { [lsearch -exact $gPo(imgList) $curFile] < 0 } {
            set gPo(imgList) [linsert $gPo(imgList) 0 $curFile]
	    set gPo(imgList) [lrange $gPo(imgList) 0 \
			      [expr $gPo(imgListLen) -1]]
	}
	AddRecentFiles $gPo(lopenMenu) left  true
	AddRecentFiles $gPo(ropenMenu) right true
    }
    $canvasId config -cursor crosshair
}

proc SaveLeftImg { { useLastDir true } } {
    global gPo

    if { ! [info exists gPo($gPo(left),photo)] } {
	return
    }
    set defName [file tail $gPo($gPo(left),name)]
    set imgName [GetFileName "Save left image" $useLastDir "save" $defName]
    if { $imgName != "" } {
	SaveImg .po $gPo($gPo(left),photo) $imgName
    }
    focus .po
}

proc SaveRightImg { { useLastDir true } } {
    global gPo

    if { ! [info exists gPo($gPo(right),photo)] } {
	return
    }
    set defName [file tail $gPo($gPo(right),name)]
    set imgName [GetFileName "Save right image" $useLastDir "save" $defName]
    if { $imgName != "" } {
	SaveImg .po $gPo($gPo(right),photo) $imgName
    }
    focus .po
}

proc SaveDiffImg { tw photoId { useLastDir true } } {
    global gPo

    set defLeftName  [file tail $gPo($gPo(left),name)]
    set defRightName [file tail $gPo($gPo(right),name)]
    set defName [format "%s_%s" [file rootname $defLeftName] $defRightName]
    set imgName [GetFileName "Save difference image" $useLastDir "save" $defName]
    if { $imgName != "" } {
...
proc OpenLeftImgFile { { useLastDir true } } {
    global gPo

    set imgName [GetFileName "Open left image" $useLastDir]
    if { $imgName != "" } {
        ReadImg $imgName $gPo(left)
    }
    focus .po
}

proc OpenRightImgFile { { useLastDir true } } {
    global gPo

    set imgName [GetFileName "Open right image" $useLastDir]
    if { $imgName != "" } {
        ReadImg $imgName $gPo(right)
    }
    focus .po
}

proc ClearImg { canvasId } {
    global gPo

    if { [info exists gPo($canvasId,photo)] } {
	image delete $gPo($canvasId,photo)
	unset gPo($canvasId,photo)
        set gPo($canvasId,name) "No image loaded"
	::poWin::SetSyncTitle $canvasId \
			      $gPo($gPo(left),name) $gPo($gPo(right),name)
    }
}
...
proc SaveImg { tw photoId imgName } {
    ...
    $gPo(left)  config -cursor watch
    $gPo(right) config -cursor watch
    update
...
proc CreateInfoWin { leftFile rightFile leftAttr rightAttr } {
...
proc PrintHistoVal { histoImgNo canvasId x y } {
    global gPo

    set sx [::poMisc::Min $x 255]
    foreach colorName { Red Green Blue } {
	set canvasId $gPo($histoImgNo,l$colorName,canvas)
	set val [lindex $gPo($histoImgNo,$canvasId,list) $sx]
...
proc PrintHistoVal2 { histoImgNo canvasId x y } {
    global gPo

    set sx [::poMisc::Min $x 255]
    foreach side { l r } {
	foreach colorName { Red Green Blue } {
	    set canvasId $gPo($histoImgNo,$side$colorName,canvas)
	    set val [lindex $gPo($histoImgNo,$canvasId,list) $sx]
	    $canvasId raise value
	    $canvasId itemconfigure value -text \
		      [format "Intensity %3d: %d values" $sx $val]
...
proc CleanUp { tw args } {
    ...
    foreach l [array names gPo "*,$tw.*,*"] {
	unset gPo($l)
    }
}

proc ShowHistogram2 { useLogScale } {
    global gPo gHistoImgNo RED GREEN BLUE
    ...
    if { $gPo($gPo(left),width) != $gPo($gPo(right),width) || \
         $gPo($gPo(left),height) != $gPo($gPo(right),height) } {
	tk_messageBox -type ok -icon warning \
	    -message "Images differ in size. No histogram possible."
	focus .po
	return
    }

    set w $gPo($gPo(left),width)
    set h $gPo($gPo(left),height)

    if { ! [info exists gHistoImgNo] } {
	set gHistoImgNo 0
    } else {
	incr gHistoImgNo
    }

    set tw .po:HistoImg$gHistoImgNo
    toplevel $tw
    wm title $tw "Histogram: [file tail $gPo($gPo(left),name)] versus \
			     [file tail $gPo($gPo(right),name)]"
    focus $tw

    label $tw.left  -text "Left image"
    label $tw.right -text "Right image"
    grid x $tw.left $tw.right
	...
	label $tw.c$color -text $color
	canvas $tw.l$color -width $hw -height $hh -borderwidth 0
	canvas $tw.r$color -width $hw -height $hh -borderwidth 0
	grid $tw.c$color $tw.l$color $tw.r$color -row $row -ipadx 2 -ipady 4
	$tw.l$color create rectangle 0 0 $hw $hh -fill white
	$tw.r$color create rectangle 0 0 $hw $hh -fill white
	$tw.l$color create text 10 10 -anchor nw -tag value
	$tw.r$color create text 10 10 -anchor nw -tag value
	$tw.l$color create image 0 0 -anchor nw -tags histo -image $photoId
	$tw.r$color create image 0 0 -anchor nw -tags histo -image $photoId
	incr row
    }
    $tw.lRed create text [expr $hw/2] [expr $hh/2] -anchor nw -tag percentage

    button $tw.bok -text "OK" -command "CleanUp $tw $photoId" 
    grid $tw.bok -row $row -columnspan 3 -sticky ew

    set gPo(stopJob) 0
    set gPo(closeWindow) 0
    bind $tw <KeyPress-Escape> { StopJob }
    wm protocol $tw WM_DELETE_WINDOW "CleanUp $tw $photoId"
    $tw config -cursor watch
    update
    ...
    set rCanv $gPo(right)
    if { $gPo(ext,poImg,avail) } {
        if {![info exists gPo($lCanv,poImg)]} {
	    ::poLog::Debug "Copying left photo to poImage"
	    set gPo($lCanv,poImg) [poImage photo_img $gPo($lCanv,photo)]	
	}
	if {![info exists gPo($gPo(right),poImg)]} {
	    ::poLog::Debug "Copying right photo to poImage"
	    set gPo($rCanv,poImg) [poImage photo_img $gPo($rCanv,photo)]	
	}

	set leftImg  $gPo($lCanv,poImg)
	set rightImg $gPo($rCanv,poImg)

	set colorChanList [list $RED $GREEN $BLUE]
	foreach img [list $leftImg $rightImg] side { l r } {
	    foreach colorName { Red Green Blue } colorChan $colorChanList {

		$img histogram $colorChan histoList
		set max [lindex [lsort -integer -decreasing $histoList] 0]
		for { set x 0 } { $x < $hw } { incr x } {
		    if { [lindex $histoList $x] != 0 } {
			if { $useLogScale } {
			    set val [expr int (log10([lindex $histoList $x]) / \
					       log10($max) * $hh)]
			} else {
			    set val [expr int ([lindex $histoList $x] / \
					       double($max) * $hh)]
			    set val [::poMisc::Max $val 1]
			}
			$tw.$side$colorName create line \
				    $x $hh $x [expr $hh-$val] -fill $colorName
		    }
		}
		set gPo($gHistoImgNo,$tw.$side$colorName,list) $histoList
		set gPo($gHistoImgNo,$side$colorName,canvas) $tw.$side$colorName
		$tw.$side$colorName raise histo
		$tw.$side$colorName bind histo <Motion> \
		    "PrintHistoVal2 $gHistoImgNo $tw.$side$colorName %x %y"
	    }
	}
    } else {
	# No poImg extension available.
	# We have to do it with standard Tk commands, which can be very, 
	# very slow for images  > 256 x 256 pixels.
	set leftPhoto  $gPo($lCanv,photo)
	set rightPhoto $gPo($rCanv,photo)

	foreach side { l r } {
	    foreach colorName { Red Green Blue } {
		set gPo($gHistoImgNo,$tw.$side$colorName,list) {}
		set gPo($gHistoImgNo,$side$colorName,canvas) $tw.$side$colorName
		$tw.$side$colorName create rectangle 0 0 $hw $hh -fill white
		for { set i 0 } { $i < 256 } { incr i } {
		    set count($side,$colorName,$i) 0
		}
	    }
	}
	for { set y 0 } { $y < $h } { incr y } {
	    set percentage [expr int($y * 100.0 / $h)]
	    $tw.lRed raise percentage
	    $tw.lRed itemconfigure percentage -text [format "%d%%" $percentage]
	    for { set x 0 } { $x < $w } { incr x } {
		set left  [$leftPhoto  get $x $y]
		set right [$rightPhoto get $x $y]
		
		incr count(l,Red,[lindex $left 0])
		incr count(l,Green,[lindex $left 1])
		incr count(l,Blue,[lindex $left 2])

		incr count(r,Red,[lindex $right 0])
		incr count(r,Green,[lindex $right 1])
		incr count(r,Blue,[lindex $right 2])
	    }
	    update
	    if { $gPo(closeWindow) } {
		WriteInfoStr "Window closed by user."
		return
	    }
	    if { $gPo(stopJob) } {
		WriteInfoStr "Image processing stopped by user."
		break
	    }
	}
	$tw.lRed delete percentage

	foreach side { l r } {
	    foreach color { Red Green Blue } {
		set max 0
		for { set i 0 } { $i < 256 } { incr i } {
		    set max [::poMisc::Max $count($side,$color,$i) $max]
		    lappend gPo($gHistoImgNo,$tw.$side$color,list) \
			    $count($side,$color,$i)
		}
		for { set i 0 } { $i < 256 } { incr i } {
		    if { $count($side,$color,$i) != 0 } {
			if { $useLogScale } {
			    set val [expr int(log10($count($side,$color,$i)) / \
					      log10($max) * $hh)]
			} else {
			    set val [expr int($count($side,$color,$i) / \
					      double($max) * $hh)]
			    set val [::poMisc::Max $val 1]
			}
			$tw.$side$color create line $i $hh $i [expr $hh-$val] \
					-fill $color
		    }
		}
		$tw.$side$color raise histo
		$tw.$side$color bind histo <Motion> \
			"PrintHistoVal2 $gHistoImgNo $tw.$side$colorName %x %y"
	    }
	}
    }
    $tw config -cursor crosshair
    bind $tw <KeyPress-Escape> "CleanUp $tw $photoId"
}

proc ShowHistogram { phImg useLogScale } {
    global gPo gHistoImgNo RED GREEN BLUE
    ...
    set tw .po:HistoImg$gHistoImgNo
    toplevel $tw
    wm title $tw "Histogram: Difference of [file tail $gPo($gPo(left),name)] \
			     and [file tail $gPo($gPo(right),name)]"
    focus $tw
	...
	label $tw.c$color -text $color
	canvas $tw.l$color -width $hw -height $hh -borderwidth 0
	grid $tw.c$color $tw.l$color -row $row -ipadx 2 -ipady 4
	$tw.l$color create rectangle 0 0 $hw $hh -fill white
	$tw.l$color create text 10 10 -anchor nw -tag value
	$tw.l$color create image 0 0 -anchor nw -tags histo -image $photoId
	incr row
    ...
    button $tw.bok -text "OK" -command "CleanUp $tw $photoId" 
    grid $tw.bok -row $row -columnspan 2 -sticky ew
    ...
    if { $gPo(ext,poImg,avail) } {
	set poImg [poImage photo_img $phImg] 

	set colorChanList [list $RED $GREEN $BLUE]
	foreach colorName { Red Green Blue } colorChan $colorChanList {
	    $poImg histogram $colorChan histoList
	    set max [lindex [lsort -integer -decreasing $histoList] 0]
	    for { set x 0 } { $x < $hw } { incr x } {
	        if { [lindex $histoList $x] != 0 } {
	  	    if { $useLogScale } {
		    ...
		    $tw.l$colorName create line \
				    $x $hh $x [expr $hh-$val] -fill $colorName
		}
	    }
	    set gPo($gHistoImgNo,$tw.l$colorName,list) $histoList
	    set gPo($gHistoImgNo,l$colorName,canvas) $tw.l$colorName
	    $tw.l$colorName raise histo
	    $tw.l$colorName bind histo <Motion> \
		    "PrintHistoVal $gHistoImgNo $tw.l$colorName %x %y"
	}
    } else {
	# No poImg extension available.
	# We have to do it with standard Tk commands, which can be very, 
	# very slow for images  > 256 x 256 pixels.
        ...
        foreach colorName { Red Green Blue } {
  	    set gPo($gHistoImgNo,$tw.l$colorName,list) {}
	    set gPo($gHistoImgNo,l$colorName,canvas) $tw.l$colorName
	    $tw.l$colorName create rectangle 0 0 $hw $hh -fill white
	    for { set i 0 } { $i < 256 } { incr i } {
	        set count(l$colorName,$i) 0
	    }
        }
	for { set y 0 } { $y < $h } { incr y } {
	    set percentage [expr int($y * 100.0 / $h)]
		...
		incr count(lRed,[lindex $val 0])
		incr count(lGreen,[lindex $val 1])
		incr count(lBlue,[lindex $val 2])
	    }
	    update
	    if { $gPo(closeWindow) } {
		WriteInfoStr "Window closed by user."
	...
	$tw.lRed delete percentage
	    ...
	    set max 0
	    for { set i 0 } { $i < 256 } { incr i } {
		set max [::poMisc::Max $count(l$color,$i) $max]
		lappend gPo($gHistoImgNo,$tw.l$color,list) \
			$count(l$color,$i)
	    }
	    for { set i 0 } { $i < 256 } { incr i } {
		if { $count(l$color,$i) != 0 } {
		    if { $useLogScale } {
			set val [expr int(log10($count(l$color,$i)) / \
					  log10($max) * $hh)]
		    } else {
			set val [expr int($count(l$color,$i) / \
					  double($max) * $hh)]
			set val [::poMisc::Max $val 1]
		    }
		    $tw.l$color create line $i $hh $i [expr $hh-$val] \
				    -fill $color
		}
	    }
	    $tw.l$color raise histo
	    $tw.l$color bind histo <Motion> \
		    "PrintHistoVal $gHistoImgNo $tw.l$colorName %x %y"
	}
    }
    $tw config -cursor crosshair
...
proc StopJob { { closeWindow 0 } } {
    ...
    set gPo(stopJob) 1
    if { $closeWindow } {
	set gPo(closeWindow) 1
    }
}

proc ShowAdjustWin { phSrc phDest } {
    global gPo

    set tw .poImgDiff:adjustWin
    ...
    toplevel $tw
    wm title $tw "Image adjustment"
    wm resizable $tw false false

    # Generate left column with text labels.
    set row 0
    foreach labelStr { "Scale:" \
                       "Threshold:" } {
	label $tw.l$row -text $labelStr
        grid  $tw.l$row -row $row -column 0 -sticky nw
	incr row
    ...
    set varList {}
    # Generate right column with entries and buttons.
    # Part 1: Scale adjustment
    set row 0
    frame $tw.fr$row
    grid $tw.fr$row -row $row -column 1 -sticky nw

    entry $tw.fr$row.e -textvariable gPo(adjustScale) -width 3
    scale $tw.fr$row.s -from 1 -to 255 \
                       -length 255 -resolution 1 \
                       -showvalue false -orient horizontal \
                       -variable gPo(adjustScale)
    pack $tw.fr$row.e $tw.fr$row.s -side left -anchor w -in $tw.fr$row 
    bind $tw.fr$row.s <ButtonRelease-1> "AdjustImg $phSrc $phDest 1 0"
    bind $tw.fr$row.s <ButtonRelease-2> "AdjustImg $phSrc $phDest 1 0"

    set tmpList [list [list gPo(adjustScale)] [list $gPo(adjustScale)]]
    lappend varList $tmpList

    # Part 2: Number of entries in Reopen menu
    incr row
    frame $tw.fr$row
    grid $tw.fr$row -row $row -column 1 -sticky news

    entry $tw.fr$row.e1 -textvariable gPo(adjustThres) -width 3
    scale $tw.fr$row.s1 -from 0 -to 255 \
                        -length 256 -resolution 1 \
                        -showvalue false -orient horizontal \
                        -variable gPo(adjustThres)
    pack $tw.fr$row.e1 $tw.fr$row.s1 -side left -anchor w -in $tw.fr$row 

    bind $tw.fr$row.s1 <ButtonRelease-1> "AdjustImg $phSrc $phDest 0 1"
    bind $tw.fr$row.s1 <ButtonRelease-2> "AdjustImg $phSrc $phDest 0 1"
...
proc AdjustImg { phSrc phDest useScale useThres } {
    global gPo RED GREEN BLUE ADD

    if { ($useScale && $gPo(adjustScale) == 1) } {
        $phDest copy $phSrc
    } elseif { $gPo(ext,poImg,avail) } {
        set poImg [poImage photo_img $phSrc]	
	drawmode_rgb $ADD $ADD $ADD
        if { $useScale } {
            $poImg change_gamma $::RED   1.0 $gPo(adjustScale)
            $poImg change_gamma $::GREEN 1.0 $gPo(adjustScale)
            $poImg change_gamma $::BLUE  1.0 $gPo(adjustScale)
        } elseif { $useThres } {
            $poImg change_gamma $::RED   1.0 255 [expr -1 * $gPo(adjustThres)]
            $poImg change_gamma $::GREEN 1.0 255 [expr -1 * $gPo(adjustThres)]
            $poImg change_gamma $::BLUE  1.0 255 [expr -1 * $gPo(adjustThres)]
        }
	$poImg img_photo $phDest
	deleteimg $poImg
    } else {
	# No poImg extension available.
	tk_messageBox \
            -message "The poImage extension is needed for image adjust." \
            -type ok -icon warning
    }
}

proc ShowDiffImg { { colorCoded 0 } } {
    global gPo gDiffImgNo
    global SUB XOR ADD
    ...
    set w [::poMisc::Max $gPo($gPo(left),width)  $gPo($gPo(right),width)]
    set h [::poMisc::Max $gPo($gPo(left),height) $gPo($gPo(right),height)]
    set wz [expr int ($w * $gPo(zoom))]
    set hz [expr int ($h * $gPo(zoom))]

    if { ! [info exists gDiffImgNo] } {
	set gDiffImgNo 0
    } else {
	incr gDiffImgNo
    }

    set tw .po:DiffImg$gDiffImgNo
    if { [catch {toplevel $tw -visual truecolor}] } {
        toplevel $tw
    }
    wm title $tw "Difference image"
    focus $tw

    frame $tw.toolfr -relief sunken -borderwidth 1
    frame $tw.workfr -relief sunken -borderwidth 1
    pack $tw.toolfr -side top -fill x
    pack $tw.workfr -side top -fill both -expand 1

    frame $tw.workfr.imgfr  -relief raised -borderwidth 1
    frame $tw.workfr.infofr -relief sunken -borderwidth 1

    grid $tw.workfr.imgfr  -row 0 -column 0 -sticky nwse
    grid $tw.workfr.infofr -row 1 -column 0 -sticky nwse 
    grid rowconfigure $tw.workfr 0 -weight 1
    grid columnconfigure $tw.workfr 0 -weight 1

    set labelStr [format "%s versus %s" [file tail $gPo($gPo(left),name)] \
					[file tail $gPo($gPo(right),name)]]
    set canvasId [::poWin::CreateScrolledCanvas $tw.workfr.imgfr $labelStr \
		  -width $wz -height $hz]
    set photoId [image create photo -width $w -height $h]
    $canvasId create image 0 0 -anchor nw -image $photoId -tags $canvasId
    $canvasId create text [expr $wz /2] [expr $hz / 2] -anchor w -tags percentage
    set phSave [image create photo -width $w -height $h]

    label $tw.workfr.infofr.label -text "Pixel values" -anchor w
    pack  $tw.workfr.infofr.label -fill x -in $tw.workfr.infofr
    set labelId $tw.workfr.infofr.label
    ...
    set fileMenu $hMenu.file
    menu $fileMenu -tearoff 0
    AddMenuCmd $fileMenu "Save as ..." "Ctrl+S" "SaveDiffImg $tw $photoId"
    AddMenuCmd $fileMenu "Close"       "Ctrl+W" "CleanUp $tw $photoId $phSave"

    bind $tw <Control-s> "SaveDiffImg $tw $photoId"
    bind $tw <Control-w> "CleanUp $tw $photoId $phSave"
    wm protocol $tw WM_DELETE_WINDOW "CleanUp $tw $photoId $phSave"
    ...
    menu $zoomMenu -tearoff 0
    AddMenuRadio $zoomMenu " 20%"  "" gPo($canvasId,zoomFactor) 0.20 \
                 "Zoom $photoId $canvasId 0.20"
    AddMenuRadio $zoomMenu " 25%"  "" gPo($canvasId,zoomFactor) 0.25 \
                 "Zoom $photoId $canvasId 0.25"
    AddMenuRadio $zoomMenu " 33%"  "" gPo($canvasId,zoomFactor) 0.33 \
                 "Zoom $photoId $canvasId 0.33"
    AddMenuRadio $zoomMenu " 50%"  "" gPo($canvasId,zoomFactor) 0.50 \
                 "Zoom $photoId $canvasId 0.50"
    AddMenuRadio $zoomMenu "100%"  "" gPo($canvasId,zoomFactor) 1.00 \
                 "Zoom $photoId $canvasId 1.00"
    AddMenuRadio $zoomMenu "200%"  "" gPo($canvasId,zoomFactor) 2.00 \
                 "Zoom $photoId $canvasId 2.00"
    AddMenuRadio $zoomMenu "300%"  "" gPo($canvasId,zoomFactor) 3.00 \
                 "Zoom $photoId $canvasId 3.00"
    AddMenuRadio $zoomMenu "400%"  "" gPo($canvasId,zoomFactor) 4.00 \
                 "Zoom $photoId $canvasId 4.00"
    AddMenuRadio $zoomMenu "500%"  "" gPo($canvasId,zoomFactor) 5.00 \
                 "Zoom $photoId $canvasId 5.00"

    menu $histoMenu -tearoff 0
    AddMenuCmd $histoMenu "Uniform scaling"     "Shift+H" \
               "ShowHistogram $phSave 0"
    AddMenuCmd $histoMenu "Logarithmic scaling" "Ctrl+H" \
               "ShowHistogram $phSave 1"
    bind $tw <H>         "ShowHistogram $phSave 0"
    bind $tw <Control-h> "ShowHistogram $phSave 1"

    AddMenuCmd $viewMenu "Adjust ..." "Ctrl+J" "ShowAdjustWin $phSave $photoId"
    bind $tw <Control-j> "ShowAdjustWin $phSave $photoId"
    ...
    ::poToolbar::AddButton $btnFrame.save [::poBmpData::save] \
	    "SaveDiffImg $tw $photoId" \
	    "Save image to file" -activebackground white
    ...
    ::poToolbar::AddButton $btnFrame.reset [::poBmpData::hundred] \
	    ...
	    "Reset image zoom" -activebackground white
    ::poToolbar::AddButton $btnFrame.histo [::poBmpData::histo] \
	    "ShowHistogram $phSave 1" "Show logarithmic histogram" \
	    -activebackground white
    ::poToolbar::AddButton $btnFrame.adjust [::poBmpData::lightsource] \
	    "ShowAdjustWin $phSave $photoId" "Show image adjust window" \
	    -activebackground white
	...
	set leftImg  $gPo($lCanv,poImg)
	set rightImg $gPo($rCanv,poImg)
	set diff1Img [poImage allocimage $w $h]
	set diff2Img [poImage allocimage $w $h]
	$diff1Img clear_black
	$diff2Img clear_black
	drawmode_rgb $SUB $SUB $SUB
	$diff1Img copy_image $leftImg
	$diff1Img copy_rectangle $rightImg 0 0 $w $h 0 0
	$diff2Img copy_image $rightImg
	$diff2Img copy_rectangle $leftImg 0 0 $w $h 0 0
	drawmode_rgb $ADD $ADD $ADD
	$diff2Img copy_rectangle $diff1Img 0 0 $w $h 0 0
	$diff2Img img_photo $photoId
	deleteimg $diff1Img
	deleteimg $diff2Img
    } else {
	# No poImg extension available.
    ...
    Zoom $photoId $canvasId $gPo(zoom)
    $canvasId config -cursor top_left_arrow
    bind $tw <KeyPress-Escape> "CleanUp $tw $photoId $phSave"
    bind $canvasId <Motion> "PrintDiffPixelValue $canvasId $photoId $labelId %x %y"
    AdjustImg $phSave $photoId [expr !$colorCoded] $colorCoded
}
...
proc ShowZoomRect {} {
    ...
    if { $gPo($gPo(left),width) != $gPo($gPo(right),width) || \
         $gPo($gPo(left),height) != $gPo($gPo(right),height) } {
	tk_messageBox -type ok -icon warning \
	    -message "Images differ in size. No zoom rectangle possible."
	set gPo(zoomRectExists) 0
	focus .po
	return
    }

    set tw .poImgdiff:ZoomRect
    set gPo(zoomRect) $tw
    ...
    set h $w
    set lCanv [canvas $tw.workfr.lcanv -relief ridge -width $w -height $h]
    set rCanv [canvas $tw.workfr.rcanv -relief ridge -width $w -height $h]
    pack $lCanv $rCanv -side left -padx 1
    set lPhoto [image create photo -width $w -height $h]
    set rPhoto [image create photo -width $w -height $h]
    $lCanv create image 0 0 -anchor nw -image $lPhoto -tags $lPhoto
    $rCanv create image 0 0 -anchor nw -image $rPhoto -tags $rPhoto
    set gPo(zoomRectExists) 1
    set gPo(zoomRect,lCanv) $lCanv
    set gPo(zoomRect,rCanv) $rCanv
    set gPo(zoomRect,lPhoto) $lPhoto
    set gPo(zoomRect,rPhoto) $rPhoto
...
proc UpdateZoomRect { canvasId cx cy sx sy } {
	...
	set leftSrc  $gPo($gPo(left),photo)
	set rightSrc $gPo($gPo(right),photo)
	set leftDst  $gPo(zoomRect,lPhoto)
	set rightDst $gPo(zoomRect,rPhoto)
	...
	if { [catch {$leftDst  copy $leftSrc  -from $x1 $y1 $x2 $y2 -to 0 0 \
	                                      -zoom $gPo(zoomRectFactor)}] } {
	    $leftDst blank
	}
	if { [catch {$rightDst copy $rightSrc -from $x1 $y1 $x2 $y2 -to 0 0 \
	                                      -zoom $gPo(zoomRectFactor)}] } {
	    $rightDst blank
	    lower $gPo(zoomRect)
        } else {
...
proc ImgInfo {} {
    ...
    set leftCanvas  $gPo(left)
    set rightCanvas $gPo(right)
    set leftAttr  [::poMisc::FileInfo $gPo($leftCanvas,name)]
    set rightAttr [::poMisc::FileInfo $gPo($rightCanvas,name)]

    lappend leftAttr  [list "Width"  $gPo($leftCanvas,width)]
    lappend leftAttr  [list "Height" $gPo($leftCanvas,height)]
    lappend rightAttr [list "Width"  $gPo($rightCanvas,width)]
    lappend rightAttr [list "Height" $gPo($rightCanvas,height)]
    CreateInfoWin $gPo($leftCanvas,name) $gPo($rightCanvas,name) \
		  $leftAttr $rightAttr
}
...
proc HelpTcl {} {
    ::poSoftLogo::ShowTclLogo 0 Tk Img combobox
}
...
proc GetSettingsFile {} {
    global gPo

    set cfgName poImgdiff.cfg
    set fileList {}

    set setDir "~"
    lappend fileList [file join $setDir $cfgName]

    set setDir $gPo(startDir)
    lappend fileList [file join $setDir $cfgName]
...
proc LoadSettings {} {
    ...
    set gPo(main,x)         10
    set gPo(main,y)         30
    set gPo(miscSett,x)     100
    set gPo(miscSett,y)     50
...
proc SaveSettings {} {
	...
	if { [info exists gPo(zoomRectSett)] && \
    	...
    	if { [winfo exists .poImgDiff:miscSettWin] } {
	    puts $fp "set gPo(miscSett,x)  \{[winfo x .poImgDiff:miscSettWin]\}"
	    puts $fp "set gPo(miscSett,y)  \{[winfo y .poImgDiff:miscSettWin]\}"
	} else {
	...
	puts $fp "set gPo(imgListLen)     \{$gPo(imgListLen)\}"
	puts $fp "set gPo(imgList)        \{[lrange [lsort $gPo(imgList)] 0 \
					     [expr $gPo(imgListLen) -1]]\}"

	puts $fp "set gPo(adjustScale)    \{$gPo(adjustScale)\}"
	puts $fp "set gPo(adjustThres)    \{$gPo(adjustThres)\}"
    ...
    ::poImgtype::SaveToFile
    tk_messageBox -message "Could not save settings." \
		  -type ok -icon warning
    focus .po
}
...
proc UpdZoomRectSettings { tw e1 e2 } {
    ...
    $gPo(left)  itemconfigure LeftRect  -outline $gPo(zoomRectColor)
    $gPo(right) itemconfigure RightRect -outline $gPo(zoomRectColor)
...
proc ZoomRectSettings {} {
    ...
    set tw .poImgdiff:ZoomRectSettings
    ...
    ::poToolhelp::AddBinding $tw.fr$row.b "Click to select new color"
...
proc ShowMiscSettWin {} {
    ...
    entry $tw.fr$row.e -textvariable gPo(imgListLen) -width 3
    button $tw.fr$row.b -text "Clear list" \
	   -command "ClearRecentImgList $gPo(lopenMenu) $gPo(ropenMenu)"
    ...
    set tmpList [list [list gPo(imgListLen)] \
                      [list $gPo(imgListLen)]]
    lappend varList $tmpList
...
set gPo(appName)  "poImgdiff"	; # Name of main application
set gPo(optCfgDir) ""		; # Default: Read from user's home directory.
set gPo(startDir) [file dirname [info script]]
set gPo(optVerbose)  false
set gPo(zoom)     1.0
...
if { $argc >= 1 } {
	...
	if { [string compare -length 1 $curParam "-"] == 0 } {
	    if { [string compare $curParam "-verbose"] == 0 } {
		set gPo(optVerbose) true
	    ...
	    if { [file readable $curParam] } {
	        if { [string compare $leftFileToOpen ""] == 0 } {
		    set leftFileToOpen $curParam
		} else {
		    set rightFileToOpen $curParam
		}
	    }
	}
	incr curArg
...
::poToolhelp::Init .po:Toolbar "#FFFFB5" black
...
if { [string compare $leftFileToOpen ""] != 0 } {
    ReadImg $leftFileToOpen $gPo(left)
}
if { [string compare $rightFileToOpen ""] != 0 } {
    ReadImg $rightFileToOpen $gPo(right)
}
